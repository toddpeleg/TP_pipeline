...
SHOT_RANGE_FILEINFO_KEY = "tp_pipe_shot_range"  # 3.12.0


def _time_unit_for_fps(fps):
    # Keep in sync with pipeline_menu.py's _FPS_TO_TIME_UNIT/_time_unit_for_fps.
    named = {24: "film", 25: "pal", 30: "ntsc", 48: "show", 50: "palf", 60: "ntscf"}
    if float(fps).is_integer() and int(fps) in named:
        return named[int(fps)]
    return f"{fps}fps"
...
def early_apply_saved_settings():
    ...
    # 3.12.0: a shot scene saved with a Shot Range (Save As Start/End,
    # embedded as fileInfo "tp_pipe_shot_range" = "<start>:<end>") keeps
    # that range on open instead of the project default above. Key must
    # match pipeline_menu.py's SCENE_SHOT_RANGE_FILEINFO_KEY.
    try:
        values = cmds.fileInfo(SHOT_RANGE_FILEINFO_KEY, query=True) or []
        start_str, end_str = values[0].split(":") if values else (None, None)
        if start_str is not None:
            start, end = int(start_str), int(end_str)
            if end >= start:
                cmds.playbackOptions(minTime=start, animationStartTime=start, maxTime=end, animationEndTime=end)
                cmds.setAttr("defaultRenderGlobals.startFrame", start)
                cmds.setAttr("defaultRenderGlobals.endFrame", end)
                applied.append(f"Shot Range: {start}-{end}")
    except Exception as e:
        print(f"TP_pipe: could not apply the scene's shot range ({e})")
