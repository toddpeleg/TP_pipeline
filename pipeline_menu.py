...
This menu is built one section at a time. Currently included:
  - SETUP (3.14.0: single-file install): Install copies this script into
    the user's Maya scripts folder and adds a small marked TP_pipe block
    to userSetup.py there (any other code in that file is left alone).
    That block auto-loads this menu every time Maya starts, so students
    don't need to drag this file in each session. There is no separate
    BIAS.py plug-in or userSetup.py to ship any more -- Increment's
    version-up logic lives in this file (see "Scene increment").
"""
...
TP_PIPE_VERSION = "v3.15.0"

# Files this menu installs, and which Maya folder each one goes in.
# "scripts" -> user prefs scripts directory
# 3.14.0 -- Todd: consolidate the install to one file. BIAS.py (the
# Increment plug-in) was merged into this script, and userSetup.py is
# no longer shipped -- this script writes its own marked block into the
# user's userSetup.py instead (see "userSetup.py managed block").
FILES_TO_INSTALL = {
    "pipeline_menu.py": "scripts",
}

# Files earlier versions installed that 3.14.0+ removes on load
# (see retire_legacy_bias_plugin).
LEGACY_BIAS_PLUGIN_FILE = "BIAS.py"
...
# per release (pipeline_X.Y.Z/ with pipeline_menu.py -- the only file
# since 3.14.0; older folders also have userSetup.py and BIAS.py) plus a
# top-level latest.json:
...
def install_update_from_github(info):
    ...
    destinations = {"scripts": get_scripts_directory()}
    ...
    try:
        ...
        # 3.14.0: userSetup.py isn't shipped any more, but the new version
        # rewrites its TP_pipe block on load -- keep a copy of it too.
        user_setup_path = os.path.join(get_scripts_directory(), "userSetup.py")
        if os.path.isfile(user_setup_path):
            shutil.copy2(user_setup_path, os.path.join(backup_dir, "userSetup.py"))
...
def build_menu():
    ...
    # 3.15.0 — Todd: "change data xfer to project utilities". The four
    # separate transfer tools (Export Pipeline, Import Pipeline, File Xfer
    # Assist, and Project Sync from the Project menu) are consolidated into
    # the single Project Xfer window (Send / Receive) — see the "Project
    # Xfer" module comment. Their old functions/panels are kept below,
    # unwired, for history (the comments that follow describe them).
    cmds.menuItem(divider=True, dividerLabel="Project Utilities", parent=menu)
    cmds.menuItem(label="Project Xfer", command=lambda *a: show_project_xfer_panel(), parent=menu)
    ...
    # (3.15.0: "Export Pipeline" / "Import Pipeline" / 3.13.0's "File Xfer
    # Assist" menu items removed — all folded into Project Xfer above.)
    ...
    # 3.15.0 — Todd: Project Settings moved here from Data Manager.
    # "Project Sync" (2.34.0-3.14.0) removed from this submenu — its
    # project-to-project compare/rebuild now lives in Project Xfer >
    # Receive (From: Project Folder). show_project_sync_window is kept,
    # unwired, for history.
    cmds.menuItem(
        label="Project Settings", command=lambda *a: project_settings_window(), parent=switch_project_menu
    )
    ...
    # (3.15.0: "Save Thumbnail" moved down to the new Utility section.)
    ...
    # 3.15.0 — Todd: section renamed "Data Manager" -> "Create".
    cmds.menuItem(divider=True, dividerLabel="Create", parent=project_context_menu)
    ...
    # 3.15.0 — Todd: "Project Settings" moved up into the Project submenu;
    # "Set Project Start Frame" removed ("its basically handled in other
    # ways" — Project Settings + the per-scene shot range);
    # start_frame_settings() kept, unwired. "Set Shot Range" moved down to
    ...
    # 3.15.0 — Todd: section renamed "Asset Manager" -> "Asset Tools"
    # (the "Asset Manager" window item itself keeps its name).
    cmds.menuItem(divider=True, dividerLabel="Asset Tools", parent=project_context_menu)
    cmds.menuItem(
    ...
    # 3.15.0 — Todd: the "Customize" submenu (Project Settings / Frame
    # Range) is removed. Project Settings lives in the Project submenu now;
    # the Frame Range render override is gone entirely — Apply Render
    # Settings uses the scene's shot range / the anim scene's range / the
    # timeline instead (customize_render_frame_range() kept, unwired).

    # ---------------- UTILITY ----------------
    # 3.15.0 — Todd: new section at the bottom for the small per-scene helpers.
    cmds.menuItem(divider=True, dividerLabel="Utility", parent=project_context_menu)
    cmds.menuItem(
        # 3.12.1 — Todd: set the current shot scene's range without
        # incrementing (stamps + saves in place). Moved here in 3.15.0.
    ...
    # show_save_thumbnail_window / the "Scene thumbnails" block. Moved here in 3.15.0.
    ...
    setup_session_hooks()  # 3.14.0 — Increment override, version label, legacy BIAS cleanup, userSetup.py block
...
def get_source_directory():
    """
    The folder this script lives in on the local drive (Install copies
    pipeline_menu.py from here).
...
def _same_file(path_a, path_b):
    """True if both paths point at the same file on disk (installing the running copy onto itself)."""
    try:
        return os.path.isfile(path_a) and os.path.isfile(path_b) and os.path.samefile(path_a, path_b)
    except Exception:
        return False


def _copy_install_files(source_dir):
    """
    Copy FILES_TO_INSTALL from source_dir into their Maya folders. Returns
    (installed, missing, failed). A file that is already the installed copy
    (Install clicked from the installed menu) counts as installed, not a
    problem.
    """
    destinations = {"scripts": get_scripts_directory()}
    ...
    for filename, dest_key in FILES_TO_INSTALL.items():
        ...
        if not os.path.isfile(src_path):
            ...
            continue
        if _same_file(src_path, dst_path):
            installed.append(f"{filename} (already installed at {dest_dir})")
            continue
        try:
...
def _print_install_report(verb, source_dir, installed, missing, failed, user_setup_note):
    print(f"{verb} {len(installed)} file(s):")
    ...
    if failed:
        ...
        for name, err in failed:
            print(f"  {name}: {err}")
    if user_setup_note:
        print(f"  userSetup.py: {user_setup_note}")


def install_setup_files():
    """
    3.14.0 single-file install: copy pipeline_menu.py into the user
    prefs scripts folder, add/refresh the TP_pipe block in userSetup.py
    there (other code in that file is kept), and remove the old BIAS.py
    plug-in if an earlier version installed it.
    """
    source_dir = get_source_directory()
    if not source_dir:
        return

    installed, missing, failed = _copy_install_files(source_dir)

    retire_legacy_bias_plugin()
    user_setup_note = ""
    if not missing and not failed:
        user_setup_note = ensure_user_setup_block()

    _print_install_report("Installed", source_dir, installed, missing, failed, user_setup_note)
    ...
    cmds.confirmDialog(
        ...
        message=(
            ...
            f"userSetup.py: {user_setup_note or 'not changed'}\n\n"
...
def update_pipeline():
    ...
    first: select the updated pipeline_menu.py file directly, install it
    into the Maya scripts folder, then immediately reload the
    freshly-installed pipeline_menu.py into THIS session and rebuild the
    menu — no Maya restart needed. (3.14.0: pipeline_menu.py is the only
    file now; the reloaded version refreshes its own userSetup.py block.)
    """
...
def _install_pipeline_from_dir(source_dir, title="Pipeline Updated", extra_note=""):
    ...
    """
    installed, missing, failed = _copy_install_files(source_dir)
    _print_install_report("Updated", source_dir, installed, missing, failed, "")
    ...
    # Loading it runs its setup_session_hooks(), which also refreshes the
    # userSetup.py block and retires an old BIAS.py.
    try:
...
def uninstall_pipeline_from_menu():
    ...
    # 3.14.0: give File > Increment and Save back to Maya and drop the
    # render Version Label callback.
    remove_increment_override()
    ...
    removed_files = []
    if retire_legacy_bias_plugin(quiet=True):
        removed_files.append(LEGACY_BIAS_PLUGIN_FILE)
    menu_path = os.path.join(scripts_dir, "pipeline_menu.py")
    if os.path.isfile(menu_path):
        try:
            os.remove(menu_path)
            removed_files.append("pipeline_menu.py")
        except Exception as e:
            cmds.warning(f"Could not remove pipeline_menu.py: {e}")

    # Strip only the TP_pipe block (or an older version's TP_pipe
    # sections) out of userSetup.py, leaving any other code untouched.
    user_setup_note = remove_user_setup_block()
    ...
    print(
        ...
        f"Files removed: {removed_files}. userSetup.py: {user_setup_note}. "
    ...
    message = (
        ...
        "- File > Increment and Save handed back to Maya\n"
        f"- Files removed: {', '.join(removed_files) if removed_files else '(none found)'}\n"
        f"- userSetup.py: {user_setup_note} (your other code preserved)\n\n"
    )
    if full_uninstall:
...
# ------------------------------------------------------------------
# Session hooks (3.14.0)
# ------------------------------------------------------------------
# Everything that used to come from BIAS.py and the shipped
# userSetup.py, set up each time the menu loads. All of it is safe to
# run repeatedly (drag-drop reloads, Update, the dev shelf buttons).

def setup_session_hooks():
    """Run on every menu load. Each step is isolated so one failure can't block the others."""
    try:
        import __main__
    except ImportError:
        __main__ = None

    # Retire the old plug-in BEFORE installing our override: unloading
    # BIAS re-sources Maya's own incrementAndSaveScene.mel, which would
    # otherwise wipe out the override installed below.
    for step in (retire_legacy_bias_plugin, install_increment_override, register_version_label_callback):
        try:
            step()
        except Exception as e:
            print(f"TP_pipe: {step.__name__} failed ({e})")

    # Keep userSetup.py's block current -- only once this script is
    # actually installed (a drag-dropped copy that was never installed
    # shouldn't make Maya try to autoload a file that isn't there).
    try:
        if os.path.isfile(os.path.join(get_scripts_directory(), "pipeline_menu.py")):
            note = ensure_user_setup_block()
            if note and note != "up to date":
                print(f"TP_pipe: userSetup.py {note}")
    except Exception as e:
        print(f"TP_pipe: could not update userSetup.py ({e})")

    # The old userSetup.py applied saved settings on SceneOpened from the
    # very start of Maya. The new block only loads this menu (deferred),
    # so a scene opened as part of Maya's startup (double-clicked .ma) can
    # already be open by now -- apply to it once, on the session's first
    # load only, so reloads never reset a timeline someone changed.
    if __main__ is not None and not getattr(__main__, "_tp_pipe_session_loaded", False):
        __main__._tp_pipe_session_loaded = True
        try:
            if cmds.file(query=True, sceneName=True):
                apply_saved_settings()
        except Exception as e:
            print(f"TP_pipe: could not apply settings to the open scene ({e})")


def retire_legacy_bias_plugin(quiet=False):
    """
    Remove the BIAS.py plug-in earlier versions installed: turn off its
    autoload, unload it, and delete it from the plug-ins folder. Its job
    is done by increment_and_save_scene now. Returns True if the file was
    deleted.
    """
    for name in ("BIAS", LEGACY_BIAS_PLUGIN_FILE):
        try:
            if cmds.pluginInfo(name, query=True, loaded=True):
                try:
                    cmds.pluginInfo(name, edit=True, autoload=False)
                except Exception:
                    pass
                cmds.unloadPlugin(name, force=True)
                if not quiet:
                    print("TP_pipe: unloaded the old BIAS plug-in (Increment is built in now).")
                break
        except Exception:
            continue

    # Build the path by hand -- get_plugins_directory() would create an
    # empty plug-ins folder just to look in it.
    version_dir = os.path.abspath(os.path.join(cmds.internalVar(userPrefDir=True), os.pardir))
    plugin_path = os.path.join(version_dir, "plug-ins", LEGACY_BIAS_PLUGIN_FILE)
    if not os.path.isfile(plugin_path):
        return False
    try:
        os.remove(plugin_path)
        if not quiet:
            print(f"TP_pipe: removed the old {LEGACY_BIAS_PLUGIN_FILE} from {os.path.dirname(plugin_path)}")
        return True
    except Exception as e:
        cmds.warning(f"Could not remove the old {LEGACY_BIAS_PLUGIN_FILE}: {e}")
        return False


# ------------------------------------------------------------------
# Scene increment (3.14.0, replaces the BIAS.py plug-in)
# ------------------------------------------------------------------
# Todd: merge BIAS into pipeline_menu.py. Our own version of what that
# plug-in did (original idea: Alex Kline's "Better Increment and Save"):
# File > Increment and Save (and TP_pipe's Increment items) bump the
# vNNN number in the scene's file name, keeping its zero padding, and
# ask what to do if that version already exists. Differences from BIAS:
#   - only the FILE NAME's version changes, never a matching "v003"
#     somewhere in the folder path;
#   - "rev2"/"dev3" aren't mistaken for versions (the v must start a
#     word), and the LAST version in the name wins;
#   - "Make Latest" uses the numerically highest existing version
#     (.ma and .mb both count), not the last file in an unsorted listing;
#   - the render Version Label is set BEFORE each save, so the saved file
#     carries its own version (BIAS set it after, one save behind).

SCENE_VERSION_PATTERN = re.compile(r"(?<![A-Za-z0-9])([vV])(\d+)(?![A-Za-z])")
MAYA_SCENE_TYPES = {".ma": "mayaAscii", ".mb": "mayaBinary"}


def _scene_version_match(file_stem):
    """The last vNNN token in a file name (no extension), or None."""
    matches = list(SCENE_VERSION_PATTERN.finditer(file_stem))
    return matches[-1] if matches else None


def _scene_stem_with_version(file_stem, match, number):
    digits = match.group(2)
    return file_stem[:match.start(2)] + str(number).zfill(len(digits)) + file_stem[match.end(2):]


def _highest_scene_version(folder, file_stem, match):
    """Highest version among <prefix>vNNN<suffix>.ma/.mb files in folder (same name apart from the number)."""
    prefix = file_stem[:match.start(2)]
    suffix = file_stem[match.end(2):]
    pattern = re.compile(rf"^{re.escape(prefix)}(\d+){re.escape(suffix)}\.(ma|mb)$", re.IGNORECASE)
...
def increment_and_save_scene():
    """
    Save the open scene as its next version (see "Scene increment").
    Returns the new file path, or None if nothing was saved.
    """
    scene_path = cmds.file(query=True, sceneName=True)
    if not scene_path:
        cmds.confirmDialog(
            title="Scene Not Saved",
            message="Save the scene first, with a version number in its name "
                    "(e.g. chair_model.v001.ma or shot_v001_desc.ma).",
            button=["OK"],
        )
        return None

    folder, file_name = os.path.split(scene_path)
    file_stem, ext = os.path.splitext(file_name)
    match = _scene_version_match(file_stem)
    if not match:
        cmds.confirmDialog(
            title="No Version Number",
            message=f"{file_name} has no version number to increment.\n\n"
                    "Use Save As to save it with one (e.g. v001) first.",
            button=["OK"],
        )
        return None

    letter, digits = match.group(1), match.group(2)
    new_path = os.path.join(folder, _scene_stem_with_version(file_stem, match, int(digits) + 1) + ext)

    if os.path.exists(new_path):
        latest_number = _highest_scene_version(folder, file_stem, match) + 1
        latest_label = f"{letter}{str(latest_number).zfill(len(digits))}"
        make_latest = f"Make Latest ({latest_label})"
        choice = cmds.confirmDialog(
            title="File Conflict",
            message=f"{os.path.basename(new_path)} already exists. What would you like to do?",
            button=["Overwrite", make_latest, "Cancel"],
            defaultButton=make_latest,
            cancelButton="Cancel",
            dismissString="Cancel",
        )
        if choice == make_latest:
            new_path = os.path.join(folder, _scene_stem_with_version(file_stem, match, latest_number) + ext)
        elif choice != "Overwrite":
            print("Increment cancelled.")
            return None

    file_type = MAYA_SCENE_TYPES.get(ext.lower())
    if not file_type:
        current_type = cmds.file(query=True, type=True) or ["mayaAscii"]
        file_type = current_type[0]

    cmds.file(rename=new_path)
    try:
        cmds.file(save=True, force=True, type=file_type)
    except Exception:
        cmds.file(rename=scene_path)  # don't leave the scene renamed to a file that was never written
        raise
    print(f"Incremented and saved: {new_path}")
    return new_path


def _tp_pipe_mel_increment():
    """Target of the incrementAndSaveScene MEL override (File > Increment and Save, its hotkey)."""
    try:
        increment_and_save_scene()
    except Exception as e:
        cmds.warning(f"Increment and Save failed: {e}")


def install_increment_override():
    """
    Point Maya's incrementAndSaveScene MEL proc (File > Increment and
    Save, and its hotkey) at increment_and_save_scene. The callable is
    parked on __main__ so the MEL proc always reaches the most recently
    loaded copy of this module (this file is loaded under several module
    names -- see userSetup.py's block and _install_pipeline_from_dir).
    """
    import __main__
    __main__._tp_pipe_increment_and_save = _tp_pipe_mel_increment
    # Source Maya's own version first so a later auto-source of it can't
    # replace ours.
    mel.eval('source "incrementAndSaveScene.mel";')
    mel.eval(
        'global proc incrementAndSaveScene(int $force) '
        '{ python("import __main__; __main__._tp_pipe_increment_and_save()"); }'
    )


def _before_save_set_version_label(*_args):
    """kBeforeSave: copy the scene file name's version (e.g. v004) into the render settings' Version Label."""
    try:
        scene_path = cmds.file(query=True, sceneName=True)
        if not scene_path:
            return
        file_stem = os.path.splitext(os.path.basename(scene_path))[0]
        match = _scene_version_match(file_stem)
        if not match:
            return
        label = match.group(1) + match.group(2)
        if cmds.getAttr("defaultRenderGlobals.renderVersion") != label:
            cmds.setAttr("defaultRenderGlobals.renderVersion", label, type="string")
    except Exception:
        pass


def register_version_label_callback():
    """Install _before_save_set_version_label as a kBeforeSave callback, replacing any from an earlier load."""
    import __main__
    import maya.api.OpenMaya as om2
    old_id = getattr(__main__, "_tp_pipe_version_label_cb", None)
    if old_id is not None:
        try:
            om2.MMessage.removeCallback(old_id)
        except Exception:
            pass
    __main__._tp_pipe_version_label_cb = om2.MSceneMessage.addCallback(
        om2.MSceneMessage.kBeforeSave, _before_save_set_version_label
    )


def remove_increment_override():
    """Uninstall: restore Maya's own incrementAndSaveScene and drop the Version Label callback."""
    try:
        import __main__
        import maya.api.OpenMaya as om2
        old_id = getattr(__main__, "_tp_pipe_version_label_cb", None)
        if old_id is not None:
            om2.MMessage.removeCallback(old_id)
            __main__._tp_pipe_version_label_cb = None
    except Exception:
        pass
    try:
        mel.eval('source "incrementAndSaveScene.mel";')
    except Exception as e:
        cmds.warning(f"Could not restore Maya's Increment and Save: {e}")


# ------------------------------------------------------------------
# userSetup.py managed block (3.14.0)
# ------------------------------------------------------------------
# Todd: ship one file. Instead of copying a whole userSetup.py over the
# user's (which wiped any code they already had there), this script
# keeps a marked block in it. Everything between the markers is ours and
# gets rewritten on load when it differs; everything outside is left
# alone. The block only loads this menu -- the settings-on-open and
# set-project-from-scene jobs the old userSetup.py had are already done
# by apply_saved_settings / align_maya_project (register_scene_opened_job).

USER_SETUP_BLOCK_START = "# >>> TP_pipe >>>"
USER_SETUP_BLOCK_END = "# <<< TP_pipe <<<"

USER_SETUP_BLOCK = f'''{USER_SETUP_BLOCK_START}
# Added by TP_pipe's pipeline_menu.py, which rewrites everything between
# these two marker lines -- keep your own code outside them.
# Loads the TP_pipe menu once Maya's UI is ready (skipped in batch mode).
def _tp_pipe_load_menu():
    import os
    import importlib.util
    import maya.cmds as cmds
    path = os.path.join(cmds.internalVar(userScriptDir=True), "pipeline_menu.py")
    if not os.path.isfile(path):
        print("TP_pipe menu: pipeline_menu.py is not installed in " + os.path.dirname(path))
        return
    try:
        spec = importlib.util.spec_from_file_location("tp_pipe_installed_menu", path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
    except Exception as e:
        print("TP_pipe menu: could not load pipeline_menu.py (%s)" % e)


def _tp_pipe_startup():
    import maya.cmds as cmds
    if not cmds.about(batch=True):
        cmds.evalDeferred(_tp_pipe_load_menu)


_tp_pipe_startup()
{USER_SETUP_BLOCK_END}
'''

# Sections that pre-3.14.0 installs copied in verbatim: everything from
# one of these markers to the end of the file was TP_pipe's.
_LEGACY_USER_SETUP_MARKERS = (
    "# ------------------------------------------------------------------\n# TP_pipe Pipeline Menu",
    "# ------------------------------------------------------------------\n# SVA Pipeline Menu",
)
# ...plus the set-project-from-scene job those files opened with.
_LEGACY_USER_SETUP_HEADER = re.compile(
    r"import maya\.cmds as cmds\s*\nimport os\s*\n\s*def workspaceExists\(p\):.*?"
    r"job = cmds\.scriptJob\(permanent=True, event=\[\"SceneOpened\", setProjectFromScene\]\)[ \t]*\n?",
    re.DOTALL,
)


def _strip_tp_pipe_from_user_setup(content):
    """Return userSetup.py content with every TP_pipe part removed (current block and pre-3.14.0 sections)."""
    block = re.compile(
        rf"{re.escape(USER_SETUP_BLOCK_START)}.*?{re.escape(USER_SETUP_BLOCK_END)}[ \t]*\n?", re.DOTALL
    )
    content = block.sub("", content)

    for marker in _LEGACY_USER_SETUP_MARKERS:
        idx = content.find(marker)
        if idx != -1:
            content = content[:idx]
            content = _LEGACY_USER_SETUP_HEADER.sub("", content, count=1)
            break
...
def _read_user_setup(path):
    """(content, newline) of userSetup.py, or ("", "\\n") if it doesn't exist. Raises on unreadable files."""
    if not os.path.isfile(path):
        return "", "\n"
...
def _write_user_setup(path, content, newline):
    with open(path, "w", encoding="utf-8", newline="") as f:
        f.write(content.replace("\n", newline))


def ensure_user_setup_block():
    """
    Make sure the scripts folder's userSetup.py has the current TP_pipe
    block (and no older TP_pipe sections), keeping any other code. Writes
    only when something changed; the previous file is backed up to
    scripts/tp_pipe_backup/userSetup.py.bak first. Returns a short note.
    """
    path = os.path.join(get_scripts_directory(), "userSetup.py")
    try:
        current, newline = _read_user_setup(path)
    except UnicodeDecodeError:
        return "left alone (not UTF-8 text, add the TP_pipe block by hand)"

    other_code = _strip_tp_pipe_from_user_setup(current)
    wanted = (other_code + "\n\n\n" if other_code else "") + USER_SETUP_BLOCK
    if current == wanted:
        return "up to date"

    if os.path.isfile(path):
        backup_dir = os.path.join(get_scripts_directory(), "tp_pipe_backup")
        os.makedirs(backup_dir, exist_ok=True)
        shutil.copy2(path, os.path.join(backup_dir, "userSetup.py.bak"))
        note = "TP_pipe block updated"
    else:
        note = "created with the TP_pipe block"
    _write_user_setup(path, wanted, newline)
    return note


def remove_user_setup_block():
    """Uninstall: take every TP_pipe part out of userSetup.py; delete the file if nothing else is left. Returns a note."""
    path = os.path.join(get_scripts_directory(), "userSetup.py")
    if not os.path.isfile(path):
        return "not found"
    try:
        ...
        return "left alone (not UTF-8 text)"
    other_code = _strip_tp_pipe_from_user_setup(current)
    if other_code == current.strip():
        return "no TP_pipe block found"
    try:
        if other_code:
            _write_user_setup(path, other_code + "\n", newline)
            return "TP_pipe block removed"
        os.remove(path)
        return "removed (it only had TP_pipe's block)"
    except Exception as e:
        cmds.warning(f"Could not clean userSetup.py: {e}")
        return f"could not be cleaned ({e})"
...
RENDER_CAMERA_GROUP = "_rendercamera"


def _shot_context_from_any_scene_path(scene_path):
    """
    3.15.0 — (project_path, shot_name) for a scene saved anywhere under
    shots/<shot>/<task>/work/maya(/scenes)/ (any shot task, lighting
    included — unlike _shot_cache_context_from_scene_path, which only
    accepts cache-producing tasks), or (None, None). Handles nested
    "Create Separate Scene Folders" shots (shots/<scene>/<shot>/...).
    """
    if not scene_path:
        return None, None
    ...
    if not maya_dir:
        return None, None
    task_dir = os.path.dirname(os.path.dirname(maya_dir))  # .../[scene/]<shot>/<task>
    shot_dir = os.path.dirname(task_dir)
    container_dir = os.path.dirname(shot_dir)
    if os.path.basename(container_dir) == "shots":
        return os.path.dirname(container_dir), os.path.basename(shot_dir)
    shots_dir = os.path.dirname(container_dir)
    if os.path.basename(shots_dir) == "shots":
        return os.path.dirname(shots_dir), f"{os.path.basename(container_dir)}/{os.path.basename(shot_dir)}"
    return None, None


def _anim_range_for_open_scene():
    """
    3.15.0 — the shot range embedded in the open scene's shot's latest
    anim scene (previs as a fallback, in case the shot never got an anim
    scene). Returns ((start, end), task_name) or (None, None).
    """
    scene_path = cmds.file(query=True, sceneName=True)
    project_path, shot_name = _shot_context_from_any_scene_path(scene_path)
    if not project_path:
        return None, None
    for task_name in ("anim", "previs"):
        try:
            shot_range, _mtime = latest_shot_task_range(project_path, shot_name, task_name, max_files=5)
        except Exception:
            shot_range = None
        if shot_range:
            return shot_range, task_name
    return None, None


def _set_render_camera_from_group():
    """
    3.15.0 — Todd: make the camera inside _rendercamera (Setup Lighting
    Scene's group) the only renderable camera. More than one camera in
    there -> ask which; none -> persp is made the renderable camera.
    Scene-level only: a render layer's own camera override still wins.
    Returns a label for the Apply Render Settings summary.
    """
    groups = cmds.ls(RENDER_CAMERA_GROUP, f"*:{RENDER_CAMERA_GROUP}", long=True) or []
    group_cameras = []
    for group in groups:
        for shape in cmds.listRelatives(group, allDescendents=True, type="camera", fullPath=True) or []:
            if shape not in group_cameras:
                group_cameras.append(shape)

    def transform_name(shape):
        parent = cmds.listRelatives(shape, parent=True) or [shape]
        return parent[0]

    note = ""
    chosen = None
    if len(group_cameras) == 1:
        chosen = group_cameras[0]
    elif len(group_cameras) > 1:
        names = [transform_name(s) for s in group_cameras]
        if len(set(names)) != len(names):  # same short name twice -> show the full paths instead
            names = [cmds.listRelatives(s, parent=True, fullPath=True)[0] for s in group_cameras]
        choice = cmds.confirmDialog(
            title="Render Camera",
            message=f"There is more than one camera in {RENDER_CAMERA_GROUP}.\nWhich one should render?",
            button=names + ["Leave As Is"],
            defaultButton=names[0],
            cancelButton="Leave As Is",
            dismissString="Leave As Is",
        )
        if choice in names:
            chosen = group_cameras[names.index(choice)]
        else:
            return "unchanged (none picked)"
    else:
        persp_shapes = cmds.listRelatives("persp", shapes=True, type="camera", fullPath=True) if cmds.objExists("persp") else None
        if not persp_shapes:
            cmds.warning(f"No camera in {RENDER_CAMERA_GROUP} and no persp camera — renderable cameras left as they were.")
            return "unchanged (no camera found)"
        chosen = persp_shapes[0]
        note = f"  (no camera found in {RENDER_CAMERA_GROUP})"

    chosen_long = (cmds.ls(chosen, long=True) or [chosen])[0]
    for shape in cmds.ls(type="camera", long=True) or []:
        try:
            cmds.setAttr(f"{shape}.renderable", shape == chosen_long)
        except Exception as e:
            cmds.warning(f"Could not set {shape}.renderable: {e}")
    label = transform_name(chosen) + note
    print(f"Render camera set to: {label}")
    return label
...
def apply_render_settings():
    ...
    Configure Arnold render settings:
      ...
      - Frame range (3.15.0): the open scene's embedded shot range, else
        the shot's latest anim scene range (then also set on this scene's
        timeline and embedded in it), else the time slider. The old
        Customize > Frame Range override is no longer used.
      - Render camera (3.15.0): the camera inside _rendercamera becomes
        the only renderable camera (persp if there isn't one)
    ...
    # Frame range (3.15.0 — Todd: "should just use the timeline or the
    # scene range we've defined in the anim scene"): the scene's embedded
    # shot range (3.12.0), else the shot's latest anim scene range —
    # which is then also set on this scene's timeline and embedded in it,
    # so the next save keeps it — else the time slider. The saved
    # Customize > Frame Range override (RENDER_FRAME_*_OPTVAR) is ignored.
    shot_range = get_open_scene_shot_range()
    range_source = "scene shot range"
    if not shot_range:
        anim_range, anim_task = _anim_range_for_open_scene()
        if anim_range:
            shot_range = anim_range
            range_source = f"{anim_task} scene (also set on this scene's timeline)"
            apply_shot_range_to_scene(*anim_range)
            _apply_shot_range_fileinfo(*anim_range)
    if shot_range:
        start_frame, end_frame = shot_range
    else:
        range_source = "timeline"
        start_frame = int(cmds.playbackOptions(query=True, minTime=True))
        end_frame = int(cmds.playbackOptions(query=True, maxTime=True))

    render_camera_label = _set_render_camera_from_group()
    ...
    summary = (
        ...
        f"Frame range: {start_frame} - {end_frame}  (from {range_source})\n"
        f"Render camera: {render_camera_label}\n"
        "Output format: name.#.ext\n"
...
def file_increment():
    ...
    try:
        ...
        align_maya_project()
        # 3.14.0: our own increment (was Maya's incrementAndSaveScene,
        # overridden by the BIAS.py plug-in) -- see "Scene increment".
        if increment_and_save_scene():
            cmds.file(modified=False)
    except Exception as e:
...
def file_increment_with_comments():
    ...
    unchanged: runs Increment and Save (same as File > Increment and
    Save -- increment_and_save_scene since 3.14.0), always creating a new version, and always
    ...
    def do_increment(comment_text):
        try:
            _apply_scene_comment_fileinfo(comment_text)
            align_maya_project()
            if increment_and_save_scene():  # 3.14.0, see "Scene increment"
                cmds.file(modified=False)
        except Exception as e:
...
def _manifest_add_asset(project_path, asset_type, asset_name):
    """
    3.15.0 — no-op. Todd dropped Project Sync's always-on tracking
    (<project>/io/manifest.json written on every Create Asset/Shot
    Folders run): Project Xfer > Receive compares against a real second
    copy — another project folder or a Send package — so it no longer
    needs a running list, and the tool stops writing that file behind the
    scenes. Kept (as a no-op) so the existing call sites stay untouched;
    existing manifest.json files are left alone and can still be picked
    as a Receive source.
    """
    return
...
def _manifest_add_shot(project_path, shot_name):
    """3.15.0 — no-op, see _manifest_add_asset."""
    return
...
# ---------------------------------------------------------------------------
# File Xfer Assist (3.13.0)
# ---------------------------------------------------------------------------
# Todd: move actual scene FILES between machines (school <-> home), not
# just folder structure (that's Export/Import Pipeline's job) — designed
# with Todd 2026-09-25 before building:
#   - Asset-Manager-style window: Assets/Shots browse on the left, "+"
#     stages into a list on the right.
#   - Collects the LATEST work scene per staged task only
#     (<task>/work/maya/scenes). "Include Publishes" checkbox (off by
#     default) also grabs the latest output/<geo|rig> publish.
#     "Include Dependencies" checkbox (off by default) scans each
#     collected scene for referenced files / file textures / alembic
#     caches and adds the ones living inside the project.
#   - Writes to <project>/io/file_xfer_assist/<YYYY-MM-DD_HHMM>/ — files
#     keep their project-relative folder tree, plus xfer_manifest.json
#     (relative path, asset/shot, task, version, checksum per file).
#   - Import lives in the same tool: point at an xfer folder, every file
#     is copied to <current project>/<relative path>. Same file already
#     there (matching checksum) is skipped silently; a DIFFERENT file at
#     the same path is flagged with Overwrite / Skip (+ "All" variants).
#   - Progress bar on both Collect and Import.
# Dependencies outside the project folder are reported, not copied (no
# project-relative home to put them in on the other machine). Paths
# stored inside the scenes themselves are left untouched — relink on the
# other machine with Texture Connect if textures come up missing.

FILE_XFER_PANEL_OBJECT_NAME = "fileXferAssistPanel"
FILE_XFER_PANEL_TITLE = "File Xfer Assist"
FILE_XFER_SUBDIR = "file_xfer_assist"  # <project>/io/<this>/<YYYY-MM-DD_HHMM>/
FILE_XFER_MANIFEST_NAME = "xfer_manifest.json"
FILE_XFER_VERSION = 1
FILE_XFER_CHUNK = 4 * 1024 * 1024

# Asset tasks that hold Maya scenes (texture is a flat, scene-less folder).
FILE_XFER_ASSET_TASKS = ("model", "rig", "lookdev", "fx")

def _file_xfer_shot_tasks():
    """Shot tasks that hold Maya scenes — same set Save As/Open Scene use (SHOT_TASK_MAYA_SUBPATH is defined further down the file, so this is looked up at call time)."""
    return tuple(SHOT_TASK_MAYA_SUBPATH.keys())

# Extensions the dependency scan cares about.
FILE_XFER_DEP_EXTENSIONS = (
    "ma", "mb", "abc", "fbx", "obj", "usd", "usda", "usdc", "vdb",
    "exr", "tx", "tif", "tiff", "png", "jpg", "jpeg", "tga", "hdr", "psd", "bmp", "iff",
)

_file_xfer_panel_instance = None


def _file_xfer_task_label(task_name):
    return ASSET_MANAGER_TASK_LABELS.get(task_name) or SHOT_TASK_LABELS.get(task_name) or task_name.capitalize()


def _file_xfer_latest(scenes_dir, stub):
    """(filename, full_path) of the newest <stub>.vNNN.(ma|mb) in scenes_dir, or (None, None)."""
    files = _list_versioned_scene_files(scenes_dir, stub)
    return files[0] if files else (None, None)


def _file_xfer_asset_latest_work(asset_dir, asset_name, task_name):
    """Latest work scene for an asset task — scenes/ first, then the pre-2.31.4 flat maya/ folder."""
    stub = f"{asset_name}_{task_name}"
    name, path = _file_xfer_latest(asset_task_scenes_dir(asset_dir, task_name), stub)
    if not name:
        name, path = _file_xfer_latest(asset_task_maya_dir(asset_dir, task_name), stub)
    return name, path


def _file_xfer_asset_latest_publish(asset_dir, asset_name, task_name):
    if task_name not in PUBLISH_OUTPUT_SUBFOLDER:
        return None, None
    return _file_xfer_latest(asset_task_source_dir(asset_dir, task_name), f"{asset_name}_{task_name}")


def _file_xfer_shot_latest_work(project_path, shot_name, task_name):
    files = open_panel_shot_scene_files(project_path, shot_name, task_name)
    return files[0] if files else (None, None)


def _file_xfer_version_of(filename):
    match = VERSIONED_FILE_PATTERN.match(filename or "")
    return int(match.group(2)) if match else None


def _file_xfer_rel(project_path, path):
    """Project-relative path with forward slashes, or None if path is outside the project."""
    root = os.path.normcase(os.path.normpath(project_path))
    full = os.path.normcase(os.path.normpath(path))
    if full != root and not full.startswith(root + os.sep):
        return None
    return os.path.relpath(os.path.normpath(path), os.path.normpath(project_path)).replace("\\", "/")


def _file_xfer_human_size(num_bytes):
...
def _file_xfer_sha256(path):
    import hashlib
    digest = hashlib.sha256()
    with open(path, "rb") as f:
        while True:
            chunk = f.read(FILE_XFER_CHUNK)
            if not chunk:
                break
            digest.update(chunk)
...
def _file_xfer_copy(src, dst, on_bytes=None):
    """
    Chunked copy that hashes as it goes (one read pass for both) and
    reports progress per chunk. on_bytes(n) may return False to cancel —
    the partial dst is removed and None is returned. Returns sha256 hex.
    """
    import hashlib
    os.makedirs(os.path.dirname(dst), exist_ok=True)
    # Written to a temp name first and swapped in only once complete, so
    # a cancel/failure mid-copy never destroys a file being overwritten.
    tmp = dst + ".fxa_tmp"
    digest = hashlib.sha256()
    cancelled = False
    try:
        with open(src, "rb") as fin, open(tmp, "wb") as fout:
            while True:
                chunk = fin.read(FILE_XFER_CHUNK)
                if not chunk:
                    break
                fout.write(chunk)
                digest.update(chunk)
                if on_bytes is not None and on_bytes(len(chunk)) is False:
                    cancelled = True
                    break
    except Exception:
        try:
...
_FILE_XFER_EXT_RE = "|".join(FILE_XFER_DEP_EXTENSIONS)
_FILE_XFER_MA_REF_RE = re.compile(r'^\s*file\s+-r[^;]*?"([^"]+)"\s*;', re.MULTILINE)
_FILE_XFER_MA_ATTR_RE = re.compile(
    r'setAttr\s+"\.(?:ftn|fn|fileTextureName|filename|fileName|abc_File|cfn)"\s+-type\s+"string"\s+"([^"]*)"'
)
_FILE_XFER_MB_PATH_RE = re.compile(
    (r'((?:[A-Za-z]:)?[/\\\\]?[\w .\-$<>{}()/\\\\]{2,400}?\.(?:' + _FILE_XFER_EXT_RE + r'))(?![\w])').encode(),
    re.IGNORECASE,
)
_FILE_XFER_TOKEN_RE = re.compile(r"<udim>|<uvtile>|<f>|<frame>|u<u>_v<v>|_u<u>_v<v>|<u>|<v>|#+", re.IGNORECASE)
_FILE_XFER_UDIM_NUM_RE = re.compile(r"(?<=[._])1\d{3}(?=\.[^.]+$)")


def _file_xfer_scene_raw_paths(scene_path):
    """Every path-looking string a scene file points at (references, textures, caches)."""
    raw = []
    try:
        if scene_path.lower().endswith(".ma"):
            with open(scene_path, "r", errors="ignore") as f:
                text = f.read()
            raw.extend(_FILE_XFER_MA_REF_RE.findall(text))
            raw.extend(_FILE_XFER_MA_ATTR_RE.findall(text))
        else:
            # .mb is binary — best-effort scan for embedded path strings.
            with open(scene_path, "rb") as f:
                data = f.read()
            for match in _FILE_XFER_MB_PATH_RE.findall(data):
                raw.append(match.decode("utf-8", "ignore"))
    except Exception as e:
        cmds.warning(f"File Xfer Assist: could not scan {scene_path} for dependencies ({e}).")
    return [p for p in raw if p and p.strip()]


def _file_xfer_resolve_dependency(raw_path, scene_path, project_path):
    """Turn one raw scene path into a list of existing files on disk (UDIM/sequence tokens expanded)."""
    import glob

    path = os.path.expandvars(raw_path.strip().replace("\\", "/"))
    path = path.split("{")[0] if path.endswith("}") else path  # Maya copy-number suffix: file.ma{1}
    if not path:
        return []
    ...
    if os.path.isabs(path) or re.match(r"^[A-Za-z]:/", path):
        ...
        candidates.append(os.path.join(project_path, path))
        try:
            maya_folder = _maya_folder_from_scene_path(scene_path)
        except Exception:
            maya_folder = None
        if maya_folder:
            candidates.append(os.path.join(maya_folder, path))
        candidates.append(os.path.join(os.path.dirname(scene_path), path))

    for candidate in candidates:
        if _FILE_XFER_TOKEN_RE.search(candidate):
            pattern = _FILE_XFER_TOKEN_RE.sub("*", candidate)
        ...
        if os.path.isfile(candidate):
            hits = [candidate]
            # A UDIM set stored as its 1001 tile — grab the sibling tiles too.
            base = os.path.basename(candidate)
            if _FILE_XFER_UDIM_NUM_RE.search(base):
                pattern = os.path.join(os.path.dirname(candidate), _FILE_XFER_UDIM_NUM_RE.sub("1[0-9][0-9][0-9]", base))
                hits = sorted(set(hits + [p for p in glob.glob(pattern) if os.path.isfile(p)]))
            return hits
    return []


def _file_xfer_scan_dependencies(scene_paths, project_path, max_depth=3):
    """
    Walk every scene's dependencies (and those scenes' own dependencies,
    e.g. a referenced publish's textures) up to max_depth levels.
    Returns (inside, outside, unresolved): inside/outside are sets of
    absolute file paths (inside/outside the project); unresolved is a set
    of raw strings that pointed at nothing on disk.
    """
    inside, outside, unresolved = set(), set(), set()
    visited = set(os.path.normcase(os.path.normpath(p)) for p in scene_paths)
    frontier = list(scene_paths)
    depth = 0
    while frontier and depth < max_depth:
        next_frontier = []
        for scene_path in frontier:
            for raw in _file_xfer_scene_raw_paths(scene_path):
                resolved = _file_xfer_resolve_dependency(raw, scene_path, project_path)
                if not resolved:
                    # .mb scans are a loose binary string match, so only
                    # report misses from .ma files (real path fields).
                    if scene_path.lower().endswith(".ma"):
                        unresolved.add(raw)
                    continue
                for dep in resolved:
                    key = os.path.normcase(os.path.normpath(dep))
                    if key in visited:
                        continue
                    visited.add(key)
                    if _file_xfer_rel(project_path, dep) is None:
                        outside.add(os.path.normpath(dep))
                    else:
                        inside.add(os.path.normpath(dep))
                    if dep.lower().endswith((".ma", ".mb")):
                        next_frontier.append(dep)
        frontier = next_frontier
        depth += 1
    return inside, outside, unresolved


# ---------------- progress ----------------

class _FileXferProgress(object):
    """QProgressDialog driven by byte count (scaled to ints so >2GB totals still work)."""

    def __init__(self, title, total_bytes, parent=None):
        self.total = max(1, int(total_bytes))
        self.done = 0
        self.dialog = QtWidgets.QProgressDialog(title, "Cancel", 0, 1000, parent)
        self.dialog.setWindowTitle(FILE_XFER_PANEL_TITLE)
    ...
    def add(self, num_bytes):
        self.done += num_bytes
        self.dialog.setValue(min(1000, int(1000.0 * self.done / self.total)))
        QtWidgets.QApplication.processEvents()
    ...
    def close(self):
        self.dialog.setValue(1000)
        self.dialog.close()


# ---------------- collect ----------------

def _file_xfer_build_collect_list(project_path, staged, include_publishes, include_dependencies):
    """
    Turn the staged entries into the concrete file list. Returns
    (files, outside, unresolved) — files is a list of dicts with src,
    rel_path, category (work/publish/thumbnail/dependency) and the
    asset/shot/task/version info that goes into the manifest.
    """
    files = []
    seen = set()

    def add(src, category, info):
        rel = _file_xfer_rel(project_path, src)
        if rel is None or not os.path.isfile(src):
            return
        key = rel.lower()
        if key in seen:
            return
        seen.add(key)
        entry = dict(info)
        entry.update({"src": src, "rel_path": rel, "category": category, "size": os.path.getsize(src)})
        files.append(entry)
    ...
    for entry in staged.values():
        info = {
            ...
            "task": entry["task"],
            "version": _file_xfer_version_of(entry["filename"]),
        }
        add(entry["path"], "work", info)
        work_scenes.append(entry["path"])
        thumb = scene_thumbnail_path(entry["path"])
        if os.path.isfile(thumb):
            add(thumb, "thumbnail", info)

        if include_publishes and entry["kind"] == "asset":
            asset_dir = os.path.join(project_path, "assets", entry["asset_type"], entry["name"])
            pub_name, pub_path = _file_xfer_asset_latest_publish(asset_dir, entry["name"], entry["task"])
            if pub_path:
                pub_info = dict(info)
                pub_info["version"] = _file_xfer_version_of(pub_name)
                add(pub_path, "publish", pub_info)

    outside, unresolved = set(), set()
    if include_dependencies:
        scan_roots = work_scenes + [f["src"] for f in files if f["category"] == "publish"]
        inside, outside, unresolved = _file_xfer_scan_dependencies(scan_roots, project_path)
        for dep in sorted(inside):
            add(dep, "dependency", {"kind": None, "name": None, "asset_type": None, "task": None, "version": None})

    return files, sorted(outside), sorted(unresolved)


def _file_xfer_collect(panel, project_path, staged, include_publishes, include_dependencies):
    QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
    try:
        files, outside, unresolved = _file_xfer_build_collect_list(
            project_path, staged, include_publishes, include_dependencies
        )
    finally:
        QtWidgets.QApplication.restoreOverrideCursor()

    if not files:
        cmds.warning("File Xfer Assist: nothing to collect — no scene files found for the staged items.")
        return False

    stamp = datetime.datetime.now().strftime("%Y-%m-%d_%H%M")
    xfer_root = os.path.join(project_path, "io", FILE_XFER_SUBDIR)
    pkg_dir = os.path.join(xfer_root, stamp)
    n = 2
    while os.path.exists(pkg_dir):  # two collects in the same minute
        pkg_dir = os.path.join(xfer_root, f"{stamp}_{n:02d}")
        n += 1
    ...
    breakdown = ", ".join(
        f"{counts[c]} {c}" for c in ("work", "publish", "dependency", "thumbnail") if counts.get(c)
    )
    message = f"Collect {len(files)} files ({_file_xfer_human_size(total)}) — {breakdown}\n\nInto:\n{pkg_dir}"
    if outside:
        message += f"\n\n{len(outside)} dependency file(s) live outside the project and will NOT be collected."
    if cmds.confirmDialog(
        title=FILE_XFER_PANEL_TITLE, message=message, button=["Collect", "Cancel"],
        defaultButton="Collect", cancelButton="Cancel", dismissString="Cancel",
    ) != "Collect":
        return False

    os.makedirs(pkg_dir)
    progress = _FileXferProgress("Collecting files...", total, parent=panel)
    manifest_files = []
    failed = []
    try:
        for i, f in enumerate(files, 1):
            progress.set_label(f"Collecting {i} of {len(files)}\n{os.path.basename(f['src'])}")
            try:
                sha = _file_xfer_copy(f["src"], os.path.join(pkg_dir, *f["rel_path"].split("/")), progress.add)
            except Exception as e:
                failed.append(f"{f['rel_path']}: {e}")
                continue
            if sha is None:  # cancelled
                break
            record = {k: v for k, v in f.items() if k != "src"}
            record["sha256"] = sha
            manifest_files.append(record)
    finally:
        was_cancelled = progress.cancelled()
        progress.close()

    if was_cancelled:
        shutil.rmtree(pkg_dir, ignore_errors=True)
        cmds.warning("File Xfer Assist: collect cancelled — partial folder removed.")
        return False
    ...
    try:
        ...
        machine = ""
    manifest = {
        "file_xfer_version": FILE_XFER_VERSION,
        "tp_pipe_version": TP_PIPE_VERSION,
        "project_name": os.path.basename(os.path.normpath(project_path)),
        "created": datetime.datetime.now().isoformat(timespec="seconds"),
        "source_machine": machine,
        "options": {"include_publishes": include_publishes, "include_dependencies": include_dependencies},
        "files": manifest_files,
        "external_dependencies_not_collected": outside,
        "unresolved_dependencies": unresolved,
    }
    with open(os.path.join(pkg_dir, FILE_XFER_MANIFEST_NAME), "w") as fh:
        json.dump(manifest, fh, indent=2)

    summary = f"Collected {len(manifest_files)} files ({_file_xfer_human_size(total)}) into:\n{pkg_dir}"
    if outside:
        summary += "\n\nNot collected (outside the project):\n  " + "\n  ".join(outside[:15])
        if len(outside) > 15:
            summary += f"\n  ...and {len(outside) - 15} more (listed in {FILE_XFER_MANIFEST_NAME})"
    if failed:
        summary += "\n\nFailed:\n  " + "\n  ".join(failed[:15])
    summary += "\n\nCopy this folder to the other machine, then use File Xfer Assist > Import there."
    cmds.confirmDialog(title=FILE_XFER_PANEL_TITLE, message=summary, button=["OK"])
    try:
        os.startfile(pkg_dir)  # Windows: open the folder so it's easy to drag to a drive
    except Exception:
        pass
    return True
...
def _file_xfer_import(panel, project_path):
    start_dir = os.path.join(project_path, "io", FILE_XFER_SUBDIR)
    result = cmds.fileDialog2(
        fileMode=3, caption="Select a File Xfer folder", okCaption="Select",
        startingDirectory=start_dir if os.path.isdir(start_dir) else project_path,
    )
    if not result:
        return False
    pkg_dir = result[0]
    manifest_path = os.path.join(pkg_dir, FILE_XFER_MANIFEST_NAME)
    if not os.path.isfile(manifest_path):
        cmds.confirmDialog(
            title=FILE_XFER_PANEL_TITLE,
            message=f"No {FILE_XFER_MANIFEST_NAME} in:\n{pkg_dir}\n\nPick the dated folder File Xfer Assist created.",
            button=["OK"],
        )
        return False
    try:
        ...
        cmds.warning(f"File Xfer Assist: could not read {manifest_path} ({e}).")
        return False

    entries = manifest.get("files", [])
    if not entries:
        cmds.warning("File Xfer Assist: that xfer folder lists no files.")
        return False

    src_project = manifest.get("project_name") or "?"
    dest_name = os.path.basename(os.path.normpath(project_path))
    message = (
        f"Import {len(entries)} files from '{src_project}'\n"
        f"(collected {manifest.get('created', '?')}"
        + (f" on {manifest['source_machine']}" if manifest.get("source_machine") else "")
        + f")\n\ninto the current project:\n{project_path}"
    )
    if src_project != dest_name:
        message += f"\n\nNote: the xfer came from a project named '{src_project}', not '{dest_name}'."
    if cmds.confirmDialog(
        title=FILE_XFER_PANEL_TITLE, message=message, button=["Import", "Cancel"],
        defaultButton="Import", cancelButton="Cancel", dismissString="Cancel",
    ) != "Import":
        return False

    # Pre-pass: sort into to-copy / identical / collisions / missing
    # BEFORE the progress bar starts, so the overwrite prompts never
    # interleave with the copy.
    to_copy, identical, collisions, missing = [], [], [], []
    QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
    try:
        for entry in entries:
            ...
            if not parts or ".." in parts:
                continue
            src = os.path.join(pkg_dir, *parts)
            dst = os.path.join(project_path, *parts)
            ...
            if os.path.isfile(dst):
                same = (
                    os.path.getsize(dst) == entry.get("size", os.path.getsize(src))
                    and entry.get("sha256")
                    and _file_xfer_sha256(dst) == entry["sha256"]
                )
                (identical if same else collisions).append(item)
            else:
                to_copy.append(item)
    finally:
        QtWidgets.QApplication.restoreOverrideCursor()
    ...
    skipped = []
    blanket = None  # "Overwrite All" / "Skip All"
    for item in collisions:
        rel = item[0]["rel_path"]
        choice = blanket
        if choice is None:
            choice = cmds.confirmDialog(
                title=f"{FILE_XFER_PANEL_TITLE} — File Exists",
                message=f"A different version of this file already exists:\n\n{rel}\n\nOverwrite it?",
                button=["Overwrite", "Skip", "Overwrite All", "Skip All"],
                defaultButton="Skip", cancelButton="Skip", dismissString="Skip",
            )
            if choice in ("Overwrite All", "Skip All"):
                blanket = choice
        if choice in ("Overwrite", "Overwrite All"):
            to_copy.append(item)
        else:
            skipped.append(rel)

    # Make sure every asset/shot task being filled in has its full
    # standard folder skeleton (workspace.mel etc.), and tell Project
    # Sync about it — same builders every other creation flow uses.
    for entry, _src, _dst in to_copy:
        task = entry.get("task")
        name = entry.get("name")
        if entry.get("category") not in ("work", "publish") or not name or not task:
            continue
        try:
            if entry.get("kind") == "asset" and entry.get("asset_type"):
                build_asset_task_structure(
                    os.path.join(project_path, "assets", entry["asset_type"], name), tasks=[task]
                )
                _manifest_add_asset(project_path, entry["asset_type"], name)
            elif entry.get("kind") == "shot":
                shot_path = os.path.join(project_path, "shots", name)
                os.makedirs(shot_path, exist_ok=True)
                build_shot_task_structure(shot_path, tasks=[task])
                _manifest_add_shot(project_path, name)
        except Exception as e:
            cmds.warning(f"File Xfer Assist: could not build folders for {name} ({e}).")

    copied, bad_checksum, failed = [], [], []
    was_cancelled = False
    if to_copy:
        total = sum(os.path.getsize(src) for _e, src, _d in to_copy)
        progress = _FileXferProgress("Importing files...", total, parent=panel)
        try:
            for i, (entry, src, dst) in enumerate(to_copy, 1):
                progress.set_label(f"Importing {i} of {len(to_copy)}\n{os.path.basename(dst)}")
                try:
                    sha = _file_xfer_copy(src, dst, progress.add)
                except Exception as e:
                    failed.append(f"{entry['rel_path']}: {e}")
                    continue
                if sha is None:
                    break
                copied.append(entry["rel_path"])
                if entry.get("sha256") and sha != entry["sha256"]:
                    bad_checksum.append(entry["rel_path"])
        finally:
            was_cancelled = progress.cancelled()
            progress.close()

    summary = f"Imported into {dest_name}:\n\n  Copied: {len(copied)}"
    if identical:
        summary += f"\n  Already identical (skipped): {len(identical)}"
    if skipped:
        summary += f"\n  Skipped by you: {len(skipped)}"
    if was_cancelled:
        summary += "\n\nCancelled part-way — files copied before cancelling were kept."
    if missing:
        summary += "\n\nListed in the manifest but missing from the xfer folder:\n  " + "\n  ".join(missing[:15])
    if bad_checksum:
        summary += "\n\nChecksum MISMATCH (copy may be damaged — re-copy the xfer folder):\n  " + "\n  ".join(bad_checksum[:15])
    if failed:
        summary += "\n\nFailed:\n  " + "\n  ".join(failed[:15])
    external = manifest.get("external_dependencies_not_collected") or []
    if external:
        summary += f"\n\n{len(external)} dependency file(s) lived outside the source project and were not included."
    cmds.confirmDialog(title=FILE_XFER_PANEL_TITLE, message=summary, button=["OK"])
    return bool(copied)
...
def _file_xfer_stylesheet(object_name=None):
    """3.15.0: object_name lets Project Xfer reuse this same look."""
    t = AM_TOKENS
    p = object_name or FILE_XFER_PANEL_OBJECT_NAME
    return _am_stylesheet() + f"""
        QWidget#{p} {{
            background: {t['bg_panel']};
            color: {t['text_primary']};
            font-family: "Inter", "Segoe UI", sans-serif;
        ...
        QWidget#{p} #amHeader {{ background: {t['bg_header']}; }}
        QWidget#{p} QLabel#amHeaderTitle {{ color: {t['text_white']}; font-weight: bold; }}
        QWidget#{p} QLabel#amToggleLabel {{ color: {t['text_muted_alt']}; font-size: 12px; }}
        QWidget#{p} QLabel#amToggleLabelActive {{ color: {t['text_white']}; font-size: 12px; font-weight: 600; }}
        QWidget#{p} #amBrowseList {{ border-right: none; }}
        QWidget#{p} QScrollArea#amBrowseScroll,
        QWidget#{p} QScrollArea#amBrowseScroll > QWidget,
        QWidget#{p} QWidget#amBrowseListInner,
        QWidget#{p} QScrollArea#amSceneScroll,
        QWidget#{p} QScrollArea#amSceneScroll > QWidget,
        QWidget#{p} QWidget#amSceneListInner {{
            background: {t['bg_dark_column']};
            border: none;
        }}
        QWidget#{p} QWidget#amBrowseHeaderRow {{
            background: {t['bg_section_header']};
            border-bottom: 1px solid {t['border']};
        }}
        QWidget#{p} QLabel#amBrowseHeaderLabel {{ color: {t['text_muted']}; font-size: 11.5px; font-weight: 600; }}
        QWidget#{p} QLabel#amBackButton {{ color: {t['text_primary_alt']}; font-size: 11.5px; font-weight: 600; }}
        QWidget#{p} QLabel#amBackButton:hover {{ color: {t['text_white']}; }}
        QWidget#{p} QFrame#fxaFooter {{
            background: {t['bg_section_header']};
            border-top: 1px solid {t['border']};
        }}
        QWidget#{p} QCheckBox {{ color: {t['text_primary']}; font-size: 12.5px; spacing: 6px; }}
    """


class FileXferAssistPanel(QtWidgets.QWidget):
    """
    3.13.0 — see the "File Xfer Assist" module comment above. Plain
    floating QWidget (not dockable), same as Export/Import Pipeline,
    with Asset Manager's 3.7.x header-bar + Assets/Shots toggle look.
    """
    ...
    def __init__(self, project_path, parent=None):
        ...
        self.shot_name = None
        # key -> {kind, name, asset_type, task, filename, path}, in stage order
        self.staged = {}

        self.setObjectName(FILE_XFER_PANEL_OBJECT_NAME)
        self.setWindowTitle(FILE_XFER_PANEL_TITLE)
        self.setWindowFlags(QtCore.Qt.Window)
        self.setStyleSheet(_file_xfer_stylesheet())
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        title = QtWidgets.QLabel(f"{FILE_XFER_PANEL_TITLE} — Project: {project_name}")
        title.setObjectName("amHeaderTitle")
        header_layout.addWidget(title)
        header_layout.addStretch(1)
        self.assets_toggle_label = QtWidgets.QLabel("Assets")
        header_layout.addWidget(self.assets_toggle_label)
        self.root_toggle = _ToggleSwitch()
        self.root_toggle.setChecked(False)
        self.root_toggle.toggled.connect(self._on_root_toggle)
        header_layout.addWidget(self.root_toggle)
        self.shots_toggle_label = QtWidgets.QLabel("Shots")
        header_layout.addWidget(self.shots_toggle_label)
        self._update_toggle_labels()
        ...
        header_row.setObjectName("amBrowseHeaderRow")
        header_row.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        header_row_layout = QtWidgets.QHBoxLayout(header_row)
        header_row_layout.setContentsMargins(10, 4, 10, 4)
        header_row_layout.setSpacing(6)
        self.back_button = QtWidgets.QLabel("‹ Back")
        self.back_button.setObjectName("amBackButton")
        self.back_button.setCursor(QtCore.Qt.PointingHandCursor)
        self.back_button.mousePressEvent = lambda e: self._on_back()
        header_row_layout.addWidget(self.back_button)
        self.browse_header = QtWidgets.QLabel("Assets")
        self.browse_header.setObjectName("amBrowseHeaderLabel")
        header_row_layout.addWidget(self.browse_header, 1)
        browse_layout.addWidget(header_row)
        browse_scroll = QtWidgets.QScrollArea()
        browse_scroll.setObjectName("amBrowseScroll")
        browse_scroll.setWidgetResizable(True)
        browse_scroll.setFrameShape(QtWidgets.QFrame.NoFrame)
        browse_inner = QtWidgets.QWidget()
        browse_inner.setObjectName("amBrowseListInner")
        browse_inner.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        self.browse_list_layout = QtWidgets.QVBoxLayout(browse_inner)
        self.browse_list_layout.setContentsMargins(4, 4, 4, 4)
        self.browse_list_layout.setSpacing(1)
        self.browse_list_layout.addStretch(1)
        browse_scroll.setWidget(browse_inner)
        browse_layout.addWidget(browse_scroll, 1)
        body.addWidget(browse_col)

        # Staged column
        staged_col = QtWidgets.QWidget()
        staged_col.setObjectName("amSceneCol")
        staged_col.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        staged_col.setMinimumWidth(380)
        staged_layout = QtWidgets.QVBoxLayout(staged_col)
        staged_layout.setContentsMargins(0, 0, 0, 0)
        staged_layout.setSpacing(0)
        staged_header_row = QtWidgets.QWidget()
        staged_header_row.setObjectName("amBrowseHeaderRow")
        staged_header_row.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        staged_header_layout = QtWidgets.QHBoxLayout(staged_header_row)
        staged_header_layout.setContentsMargins(10, 4, 10, 4)
        self.staged_header = QtWidgets.QLabel("To Collect — 0")
        self.staged_header.setObjectName("amBrowseHeaderLabel")
        staged_header_layout.addWidget(self.staged_header, 1)
        clear_label = QtWidgets.QLabel("Clear")
        clear_label.setObjectName("amBackButton")
        clear_label.setCursor(QtCore.Qt.PointingHandCursor)
        clear_label.mousePressEvent = lambda e: self._clear_staged()
        staged_header_layout.addWidget(clear_label)
        staged_layout.addWidget(staged_header_row)
        staged_scroll = QtWidgets.QScrollArea()
        staged_scroll.setObjectName("amSceneScroll")
        staged_scroll.setWidgetResizable(True)
        staged_scroll.setFrameShape(QtWidgets.QFrame.NoFrame)
        staged_inner = QtWidgets.QWidget()
        staged_inner.setObjectName("amSceneListInner")
        staged_inner.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        self.staged_list_layout = QtWidgets.QVBoxLayout(staged_inner)
        self.staged_list_layout.setContentsMargins(0, 0, 0, 0)
        self.staged_list_layout.setSpacing(0)
        self.staged_list_layout.addStretch(1)
        staged_scroll.setWidget(staged_inner)
        staged_layout.addWidget(staged_scroll, 1)
        body.addWidget(staged_col, 1)

        # Footer
        footer = QtWidgets.QFrame()
        footer.setObjectName("fxaFooter")
        footer_layout = QtWidgets.QHBoxLayout(footer)
        footer_layout.setContentsMargins(10, 10, 10, 10)
        footer_layout.setSpacing(12)
        self.publishes_check = QtWidgets.QCheckBox("Include Publishes")
        self.publishes_check.setToolTip("Also collect each staged asset task's latest publish (output/geo or output/rig).")
        footer_layout.addWidget(self.publishes_check)
        self.deps_check = QtWidgets.QCheckBox("Include Dependencies")
        self.deps_check.setToolTip(
            "Also collect the files each scene points at (references, textures, alembic caches) "
            "that live inside this project."
        )
        footer_layout.addWidget(self.deps_check)
        footer_layout.addStretch(1)
        import_btn = QtWidgets.QPushButton("Import…")
        import_btn.setProperty("class", "amFooterCancel")
        import_btn.setCursor(QtCore.Qt.PointingHandCursor)
        import_btn.setToolTip("Pick a File Xfer folder and copy its files into the current project.")
        import_btn.clicked.connect(self._on_import)
        footer_layout.addWidget(import_btn)
        collect_btn = QtWidgets.QPushButton("Collect")
        collect_btn.setProperty("class", "amFooterUpdate")
        collect_btn.setCursor(QtCore.Qt.PointingHandCursor)
        collect_btn.clicked.connect(self._on_collect)
        footer_layout.addWidget(collect_btn)
        cancel_btn = QtWidgets.QPushButton("Cancel")
        cancel_btn.setProperty("class", "amFooterCancel")
        cancel_btn.setCursor(QtCore.Qt.PointingHandCursor)
        cancel_btn.clicked.connect(self.close)
    ...
    def _update_toggle_labels(self):
        shots = self.root_tab == "Shots"
        self.assets_toggle_label.setObjectName("amToggleLabel" if shots else "amToggleLabelActive")
        self.shots_toggle_label.setObjectName("amToggleLabelActive" if shots else "amToggleLabel")
        for label in (self.assets_toggle_label, self.shots_toggle_label):
    ...
    def _on_root_toggle(self, checked):
        self.root_tab = "Shots" if checked else "Assets"
        self.asset_type = self.asset_name = self.shot_name = None
        self._update_toggle_labels()
        self._refresh_browse()

    def _on_back(self):
        if self.root_tab == "Shots":
            self.shot_name = None
    ...
    def _add_browse_row(self, row):
        self.browse_list_layout.insertWidget(self.browse_list_layout.count() - 1, row)

    def _add_browse_note(self, text):
        label = QtWidgets.QLabel(text)
        label.setWordWrap(True)
        label.setStyleSheet(f"color:{AM_TOKENS['text_muted']}; font-size:11.5px; padding:8px;")
        self._add_browse_row(label)

    def _refresh_browse(self):
        while self.browse_list_layout.count() > 1:
            widget = self.browse_list_layout.takeAt(0).widget()
            if widget:
                widget.deleteLater()

        if self.root_tab == "Shots":
            self.back_button.setVisible(self.shot_name is not None)
            if self.shot_name is None:
                self.browse_header.setText("Shots")
                shots = list_existing_shots(self.project_path)
                for shot in shots:
                    count = len(self._shot_task_scenes(shot))
                    row = _AssetManagerBrowseRow(shot, f"{count} task scene{'s' if count != 1 else ''}", has_chevron=True)
                    row.clicked.connect(lambda s=shot: self._open_shot(s))
                    self._add_browse_row(row)
                if not shots:
                    self._add_browse_note("No shots in this project yet.")
            else:
                self.browse_header.setText(self.shot_name)
                self._fill_task_rows(self._shot_task_scenes(self.shot_name), "shot", self.shot_name, None)
            return

        self.back_button.setVisible(self.asset_type is not None)
        if self.asset_type is None:
            self.browse_header.setText("Assets")
            types = list_asset_category_types(self.project_path)
            for type_name in types:
                count = len(list_all_assets(self.project_path, type_name=type_name))
                row = _AssetManagerBrowseRow(
                    type_name.capitalize(), f"{count} asset{'s' if count != 1 else ''}", has_chevron=True
                )
                row.clicked.connect(lambda t=type_name: self._open_type(t))
                self._add_browse_row(row)
            if not types:
                self._add_browse_note("No assets in this project yet.")
        elif self.asset_name is None:
            self.browse_header.setText(self.asset_type.capitalize())
            assets = list_all_assets(self.project_path, type_name=self.asset_type)
            for asset_name, asset_dir in assets:
                count = len(self._asset_task_scenes(asset_dir, asset_name))
                row = _AssetManagerBrowseRow(asset_name, f"{count} task scene{'s' if count != 1 else ''}", has_chevron=True)
                row.clicked.connect(lambda n=asset_name: self._open_asset(n))
                self._add_browse_row(row)
            if not assets:
                self._add_browse_note(f"No {self.asset_type} assets yet.")
        else:
            self.browse_header.setText(f"{self.asset_type.capitalize()} › {self.asset_name}")
            asset_dir = os.path.join(self.project_path, "assets", self.asset_type, self.asset_name)
            self._fill_task_rows(
                self._asset_task_scenes(asset_dir, self.asset_name), "asset", self.asset_name, self.asset_type
            )

    def _fill_task_rows(self, task_scenes, kind, name, asset_type):
        if not task_scenes:
            self._add_browse_note("No work scenes saved for this yet.")
            return
        all_row = _AssetManagerBrowseRow("All Tasks", f"{len(task_scenes)} latest scene(s)", has_add=True)
        all_row.add_clicked.connect(
            lambda ts=task_scenes: self._stage_many([(kind, name, asset_type, t, fn, p) for t, fn, p in ts])
        )
        self._add_browse_row(all_row)
        for task, filename, path in task_scenes:
            row = _AssetManagerBrowseRow(_file_xfer_task_label(task), filename, has_add=True)
            row.add_clicked.connect(
                lambda t=task, fn=filename, p=path: self._stage_many([(kind, name, asset_type, t, fn, p)])
            )
            self._add_browse_row(row)

    def _open_type(self, type_name):
        self.asset_type = type_name
        self._refresh_browse()

    def _open_asset(self, asset_name):
        self.asset_name = asset_name
        self._refresh_browse()

    def _open_shot(self, shot_name):
        self.shot_name = shot_name
        self._refresh_browse()

    def _asset_task_scenes(self, asset_dir, asset_name):
        found = []
        for task in FILE_XFER_ASSET_TASKS:
            filename, path = _file_xfer_asset_latest_work(asset_dir, asset_name, task)
            if filename:
                found.append((task, filename, path))
        return found

    def _shot_task_scenes(self, shot_name):
        found = []
        for task in _file_xfer_shot_tasks():
            filename, path = _file_xfer_shot_latest_work(self.project_path, shot_name, task)
            if filename:
                found.append((task, filename, path))
        return found
    ...
    def _stage_many(self, items):
        for kind, name, asset_type, task, filename, path in items:
            key = (kind, asset_type, name, task)
            self.staged[key] = {
                "kind": kind, "name": name, "asset_type": asset_type,
                "task": task, "filename": filename, "path": path,
            }
        self._refresh_staged()

    def _unstage(self, key):
    ...
    def _refresh_staged(self):
        while self.staged_list_layout.count() > 1:
            widget = self.staged_list_layout.takeAt(0).widget()
            if widget:
                widget.deleteLater()
        self.staged_header.setText(f"To Collect — {len(self.staged)}")
        for key, entry in self.staged.items():
            where = f"{entry['name']} ({entry['asset_type']})" if entry["kind"] == "asset" else entry["name"]
            row = _PipelineStagedRow(f"{where} — {_file_xfer_task_label(entry['task'])}", entry["filename"])
            row.remove_clicked.connect(lambda k=key: self._unstage(k))
            self.staged_list_layout.insertWidget(self.staged_list_layout.count() - 1, row)
    ...
    def _on_collect(self):
        if not self.staged:
            cmds.warning("File Xfer Assist: stage at least one scene first.")
            return
        done = _file_xfer_collect(
            self, self.project_path, self.staged,
            self.publishes_check.isChecked(), self.deps_check.isChecked(),
        )
        if done:
            self.close()

    def _on_import(self):
        if _file_xfer_import(self, self.project_path):
            self._refresh_browse()


def show_file_xfer_assist_panel():
    """Menu entry point — Data Xfer > File Xfer Assist."""
    global _file_xfer_panel_instance
    project_path = get_current_project()
    if not project_path:
        return None
    if _file_xfer_panel_instance is not None:
        try:
            _file_xfer_panel_instance.close()
            _file_xfer_panel_instance.deleteLater()
        except Exception:
            pass
    panel = FileXferAssistPanel(project_path)
    _file_xfer_panel_instance = panel
    _pipeline_panel_show(panel)
    return panel
...
# Project Xfer (3.15.0)
# ---------------------------------------------------------------------------
# Todd: "clean up and reorganize import pipeline, export pipeline, file xfer
# assist and project sync ... they can be consolidated into a single tool
# and made more clear." All four moved "project state from A to B"; they
# only differed in folders-vs-files and package-vs-direct. Designed with
# Todd 2026-09-26 before building:
#
# One window (TP_pipe > Project Utilities > Project Xfer), Send / Receive
# toggle in the header.
#
# SEND (replaces Export Pipeline + File Xfer Assist's Collect):
#   - Assets/Shots browse on the left. "+" on an asset/shot stages the
#     whole thing; click into it to stage single tasks.
#   - Folder structure + project settings are always included.
#   - "Scene Files" (on): latest work scene per task (+ its thumbnail).
#     Off = the old folders-and-settings-only Export Pipeline package.
#     "Include Publishes" / "Include Dependencies" (off) as before.
#   - Writes <project>/io/xfer/<YYYY-MM-DD_HHMM>/ with one
#     xfer_manifest.json (structure + settings + files), progress bar.
#
# RECEIVE (replaces Import Pipeline, File Xfer Assist's Import, and
# Project Sync):
#   - From: a package (a Send folder's xfer_manifest.json — also reads the
#     older 3.13 File Xfer Assist folders, 2.26+ Export Pipeline .json
#     files, and Project Sync's io/manifest.json) OR another project folder
#     directly (Project Sync's two-locations case).
#   - Into: the current project, the project of the same name in the
#     projects folder (created if new — skeleton built, like Import
#     Pipeline did), or any other folder.
#   - A compare list shows each asset/shot as New / Conflict / Missing
#     tasks / Newer files / Up to date, checked by default unless up to
#     date. Nothing is written until Receive.
#   - "Copy Scene Files" (project-folder source only, off by default):
#     unchecked = folders only (what Project Sync did); checked = also
#     brings over each task's latest work scene when the destination
#     doesn't already have that version or newer.
#   - "Apply Project Settings" (package source only), "Switch to Project".
#   - Never deletes. Identical files are skipped silently; a different file
#     at the same path asks Overwrite / Skip (+ "All" variants).
#
# Project Sync's always-on io/manifest.json tracking is retired in this
# same version (see _manifest_add_asset) — comparing against a real second
# copy (a project folder or an older Send package) replaces it.

PROJECT_XFER_PANEL_OBJECT_NAME = "projectXferPanel"
PROJECT_XFER_PANEL_TITLE = "Project Xfer"
PROJECT_XFER_SUBDIR = "xfer"  # <project>/io/<this>/<YYYY-MM-DD_HHMM>/
PROJECT_XFER_MANIFEST_NAME = FILE_XFER_MANIFEST_NAME  # "xfer_manifest.json" — same name old File Xfer folders used
PROJECT_XFER_VERSION = 1

# status id -> (label, row tint or None). Order = display priority.
XFER_STATUS = {
    "new": ("New", AM_TOKENS["red_bg"]),
    "conflict": ("Conflict", "#5e4424"),
    "missing_tasks": ("Missing tasks", AM_TOKENS["yellow_bg"]),
    "newer": ("Newer files", AM_TOKENS["blue_row_bg"]),
    "uptodate": ("Up to date", None),
    "source_missing": ("Files missing from package", AM_TOKENS["red_bg"]),
}

_project_xfer_panel_instance = None


# ---------------- shared helpers ----------------

def _xfer_item_dir(project_path, item):
    """The asset/shot folder an item maps to in project_path (None for the Dependencies pseudo-item)."""
    if item["kind"] == "asset":
        return os.path.join(project_path, "assets", item["asset_type"], item["name"])
    if item["kind"] == "shot":
        return os.path.join(project_path, "shots", item["name"])
    return None


def _xfer_item_label(item):
    if item["kind"] == "asset":
        return f"{item['name']}  ({item['asset_type']})"
    if item["kind"] == "shot":
        return item["name"]
    return "Dependencies"


def _xfer_valid_tasks(kind):
    return ASSET_TASKS if kind == "asset" else tuple(SHOT_TASK_STRUCTURE.keys())


def _xfer_scene_tasks(kind):
    """Tasks that hold Maya work scenes (texture has none)."""
    return FILE_XFER_ASSET_TASKS if kind == "asset" else _file_xfer_shot_tasks()


def _xfer_disk_tasks(item_dir, kind):
    if not item_dir or not os.path.isdir(item_dir):
        return []
    return [t for t in _xfer_valid_tasks(kind) if os.path.isdir(os.path.join(item_dir, t))]


def _xfer_latest_work(project_path, item, task):
    """(filename, path) of the latest work scene for one item task in project_path, or (None, None)."""
    if item["kind"] == "asset":
        return _file_xfer_asset_latest_work(_xfer_item_dir(project_path, item), item["name"], task)
    if item["kind"] == "shot":
        return _file_xfer_shot_latest_work(project_path, item["name"], task)
    return None, None
...
def _xfer_get_item(items, kind, name, asset_type=None):
    key = (kind, asset_type or "", name or "")
    if key not in items:
        items[key] = {"kind": kind, "name": name, "asset_type": asset_type, "tasks": [], "files": []}
    return items[key]


def _xfer_add_tasks(item, tasks):
    for task in tasks or []:
        if task and task not in item["tasks"]:
            item["tasks"].append(task)


def _xfer_sorted_items(items):
    order = {"asset": 0, "shot": 1, "deps": 2}
    return sorted(
        items.values(),
        key=lambda i: (order.get(i["kind"], 3), (i.get("asset_type") or "").lower(), (i.get("name") or "").lower()),
    )


# ---------------- SEND ----------------

def _xfer_send_structure(project_path, staged):
    """(assets, shots) manifest lists for the staged items — the task folders each one gets built with."""
    assets, shots = [], []
    for item in staged.values():
        if item["tasks"] is None:  # whole item: every task folder it has on disk
            tasks = _xfer_disk_tasks(_xfer_item_dir(project_path, item), item["kind"]) or list(_xfer_valid_tasks(item["kind"]))
        else:
            tasks = list(item["tasks"])
        if item["kind"] == "asset":
            assets.append({"asset_name": item["name"], "asset_type": item["asset_type"], "tasks": tasks})
        else:
            shots.append({"shot_name": item["name"], "tasks": tasks})
    return assets, shots


def _xfer_send_scene_entries(project_path, staged):
    """Latest work scene per staged task, in _file_xfer_build_collect_list's staged-entry shape."""
    entries = {}
    for item in staged.values():
        scene_tasks = _xfer_scene_tasks(item["kind"])
        tasks = scene_tasks if item["tasks"] is None else [t for t in item["tasks"] if t in scene_tasks]
        for task in tasks:
            filename, path = _xfer_latest_work(project_path, item, task)
            if filename:
                entries[(item["kind"], item["asset_type"], item["name"], task)] = {
                    "kind": item["kind"], "name": item["name"], "asset_type": item["asset_type"],
                    "task": task, "filename": filename, "path": path,
                }
    return entries


def _xfer_send(panel, project_path, staged, include_files, include_publishes, include_dependencies):
    """Write a Send package. Returns True when one was written."""
    assets, shots = _xfer_send_structure(project_path, staged)

    files, outside, unresolved = [], [], []
    if include_files:
        QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
        try:
            scene_entries = _xfer_send_scene_entries(project_path, staged)
            files, outside, unresolved = _file_xfer_build_collect_list(
                project_path, scene_entries, include_publishes, include_dependencies
            )
        finally:
            QtWidgets.QApplication.restoreOverrideCursor()

    stamp = datetime.datetime.now().strftime("%Y-%m-%d_%H%M")
    xfer_root = os.path.join(project_path, "io", PROJECT_XFER_SUBDIR)
    pkg_dir = os.path.join(xfer_root, stamp)
    n = 2
    while os.path.exists(pkg_dir):  # two sends in the same minute
        pkg_dir = os.path.join(xfer_root, f"{stamp}_{n:02d}")
        n += 1

    total = sum(f["size"] for f in files)
    message = (
        f"Send {len(assets)} asset(s) and {len(shots)} shot(s)\n"
        "(folder structure + project settings)"
    )
    if include_files:
        if files:
            counts = {}
            ...
            message += f"\n+ {len(files)} files ({_file_xfer_human_size(total)}) — {breakdown}"
        else:
            message += "\n\nNo saved scene files were found for the staged items — only folders/settings will be sent."
    message += f"\n\nInto:\n{pkg_dir}"
    if outside:
        message += f"\n\n{len(outside)} dependency file(s) live outside the project and will NOT be included."
    if cmds.confirmDialog(
        title=PROJECT_XFER_PANEL_TITLE, message=message, button=["Send", "Cancel"],
        defaultButton="Send", cancelButton="Cancel", dismissString="Cancel",
    ) != "Send":
        return False

    os.makedirs(pkg_dir)
    manifest_files, failed = [], []
    was_cancelled = False
    if files:
        progress = _FileXferProgress("Collecting files...", total, parent=panel)
        progress.dialog.setWindowTitle(PROJECT_XFER_PANEL_TITLE)
        try:
            for i, f in enumerate(files, 1):
                progress.set_label(f"Collecting {i} of {len(files)}\n{os.path.basename(f['src'])}")
                try:
    ...
    if was_cancelled:
        shutil.rmtree(pkg_dir, ignore_errors=True)
        cmds.warning("Project Xfer: send cancelled — partial folder removed.")
        return False
    ...
    try:
        ...
        machine = ""
    manifest = {
        "project_xfer_version": PROJECT_XFER_VERSION,
        "tp_pipe_version": TP_PIPE_VERSION,
        "project_name": os.path.basename(os.path.normpath(project_path)),
        "created": datetime.datetime.now().isoformat(timespec="seconds"),
        "source_machine": machine,
        "options": {
            "scene_files": include_files,
            "include_publishes": include_files and include_publishes,
            "include_dependencies": include_files and include_dependencies,
        },
        "assets": assets,
        "shots": shots,
        "settings": _current_pipeline_package_settings(),
    ...
    with open(os.path.join(pkg_dir, PROJECT_XFER_MANIFEST_NAME), "w") as fh:
        json.dump(manifest, fh, indent=2)

    summary = f"Sent {len(assets)} asset(s), {len(shots)} shot(s)"
    if include_files:
        summary += f", {len(manifest_files)} files ({_file_xfer_human_size(total)})"
    summary += f" into:\n{pkg_dir}"
    if outside:
        summary += "\n\nNot included (outside the project):\n  " + "\n  ".join(outside[:15])
        if len(outside) > 15:
            summary += f"\n  ...and {len(outside) - 15} more (listed in {PROJECT_XFER_MANIFEST_NAME})"
    if failed:
        summary += "\n\nFailed:\n  " + "\n  ".join(failed[:15])
    summary += "\n\nCopy this folder to the other machine, then use Project Xfer > Receive there."
    cmds.confirmDialog(title=PROJECT_XFER_PANEL_TITLE, message=summary, button=["OK"])
    try:
        os.startfile(pkg_dir)  # Windows: open the folder so it's easy to drag to a drive
    except Exception:
        pass
...
def _xfer_load_package(path):
    """
    Read any package format into a Receive source dict, or None:
      - Project Xfer Send folder (xfer_manifest.json, project_xfer_version)
      - 3.13 File Xfer Assist folder (xfer_manifest.json, file_xfer_version) — files only
      - 2.26+ Export Pipeline .json (pipeline_package_version) — structure + settings only
      - Project Sync's io/manifest.json ({"assets": [[type, name]], "shots": [...]}) — structure only
    """
    if os.path.isdir(path):
        path = os.path.join(path, PROJECT_XFER_MANIFEST_NAME)
    try:
        with open(path, "r") as fh:
            data = json.load(fh)
    except Exception as e:
        cmds.warning(f"Project Xfer: could not read {path} ({e}).")
        return None
    if not isinstance(data, dict):
        cmds.warning(f"Project Xfer: {path} isn't a package this tool recognizes.")
        return None

    pkg_dir = os.path.dirname(path)
    items = _xfer_new_items()
    legacy_sync = False
    for asset in data.get("assets", []) or []:
        if isinstance(asset, (list, tuple)) and len(asset) == 2:  # Project Sync manifest pair
            legacy_sync = True
            item = _xfer_get_item(items, "asset", asset[1], asset[0])
            _xfer_add_tasks(item, ASSET_TASKS)
        elif isinstance(asset, dict) and asset.get("asset_name") and asset.get("asset_type"):
            item = _xfer_get_item(items, "asset", asset["asset_name"], asset["asset_type"])
            _xfer_add_tasks(item, asset.get("tasks") or ASSET_TASKS)
    for shot in data.get("shots", []) or []:
        if isinstance(shot, str):
            legacy_sync = True
            item = _xfer_get_item(items, "shot", shot)
            _xfer_add_tasks(item, SHOT_TASK_STRUCTURE.keys())
        elif isinstance(shot, dict) and shot.get("shot_name"):
            item = _xfer_get_item(items, "shot", shot["shot_name"])
            _xfer_add_tasks(item, shot.get("tasks") or SHOT_TASK_STRUCTURE.keys())
    ...
    for f in data.get("files", []) or []:
        ...
        kind = f.get("kind")
        if kind == "asset" and f.get("name") and f.get("asset_type"):
            item = _xfer_get_item(items, "asset", f["name"], f["asset_type"])
        elif kind == "shot" and f.get("name"):
            item = _xfer_get_item(items, "shot", f["name"])
        else:
            item = _xfer_get_item(items, "deps", "Dependencies")
        if item["kind"] != "deps":
            _xfer_add_tasks(item, [entry["task"]])
        item["files"].append(entry)
        has_files = True

    if not items:
        cmds.warning(f"Project Xfer: {os.path.basename(path)} has no assets, shots or files in it.")
        return None

    if "project_xfer_version" in data:
        fmt = "Project Xfer package"
    elif "file_xfer_version" in data:
        fmt = "File Xfer Assist folder (3.13)"
    elif "pipeline_package_version" in data:
        fmt = "Export Pipeline package"
    elif legacy_sync:
        fmt = "Project Sync manifest"
    else:
        fmt = "package"

    is_folder_pkg = os.path.basename(path) == PROJECT_XFER_MANIFEST_NAME
    project_name = data.get("project_name")
    if not project_name and legacy_sync and os.path.basename(pkg_dir) == "io":
        project_name = os.path.basename(os.path.dirname(pkg_dir))
    return {
        "type": "package",
        "path": pkg_dir if is_folder_pkg else path,
        "label": f"{os.path.basename(pkg_dir) if is_folder_pkg else os.path.basename(path)}  —  {fmt}",
        "project_name": project_name,
        "project_path": None,
        "settings": data.get("settings") or None,
        "items": items,
        "has_files": has_files,
        "external": data.get("external_dependencies_not_collected") or [],
    }


def _xfer_looks_like_project(path):
    return bool(path) and (os.path.isdir(os.path.join(path, "assets")) or os.path.isdir(os.path.join(path, "shots")))


def _xfer_load_project_source(project_path, include_files):
    """Receive source straight from another project folder (Project Sync's two-locations case)."""
    items = _xfer_new_items()
    disk_assets, disk_shots = _scan_disk_assets_and_shots(project_path)
    for asset_type, asset_name in sorted(disk_assets):
        item = _xfer_get_item(items, "asset", asset_name, asset_type)
        _xfer_add_tasks(item, _xfer_disk_tasks(_xfer_item_dir(project_path, item), "asset"))
    for shot_name in sorted(disk_shots):
        item = _xfer_get_item(items, "shot", shot_name)
        _xfer_add_tasks(item, _xfer_disk_tasks(_xfer_item_dir(project_path, item), "shot"))

    if include_files:
        for item in items.values():
            for task in _xfer_scene_tasks(item["kind"]):
                if task not in item["tasks"]:
                    continue
                filename, path = _xfer_latest_work(project_path, item, task)
                if not filename:
                    continue
                version = _file_xfer_version_of(filename)
                candidates = [(path, "work")]
                thumb = scene_thumbnail_path(path)
                if os.path.isfile(thumb):
                    candidates.append((thumb, "thumbnail"))
                for src, category in candidates:
                    rel = _file_xfer_rel(project_path, src)
                    if rel is None:
                        continue
                    item["files"].append({
                        "rel_path": rel, "src": src, "size": os.path.getsize(src), "sha256": None,
                        "task": task, "version": version, "category": category,
                    })
    ...
    return {
        "type": "project",
        "path": project_path,
        "label": f"{name}  —  project folder  ({project_path})",
        "project_name": name,
...
def _xfer_cached_sha(path, cache):
    try:
        st = os.stat(path)
    except OSError:
        return None
    key = (os.path.normcase(os.path.normpath(path)), st.st_mtime, st.st_size)
    if key not in cache:
        cache[key] = _file_xfer_sha256(path)
    return cache[key]


def _xfer_same_file(entry, dst, cache):
    try:
        src_size = entry["size"] if entry.get("size") is not None else os.path.getsize(entry["src"])
        if os.path.getsize(dst) != src_size:
            return False
    except OSError:
        return False
    src_sha = entry.get("sha256") or _xfer_cached_sha(entry["src"], cache)
    return bool(src_sha) and _xfer_cached_sha(dst, cache) == src_sha


def _xfer_compare_item(item, dest_project, dest_is_project, sha_cache, latest_cache):
    """
    Compare one source item against the destination. Returns a dict:
    status, detail, exists, missing_tasks, copy [(entry, dst)],
    conflicts [(entry, dst)], identical, dest_newer, source_missing.
    """
    item_dir = _xfer_item_dir(dest_project, item)
    exists = dest_is_project and (item_dir is None or os.path.isdir(item_dir))
    missing_tasks = [] if item_dir is None else [t for t in item["tasks"] if not os.path.isdir(os.path.join(item_dir, t))]
    result = {
        "exists": exists, "missing_tasks": missing_tasks, "copy": [], "conflicts": [],
        "identical": 0, "dest_newer": 0, "source_missing": 0,
    }
    ...
    for entry in item["files"]:
        if not os.path.isfile(entry["src"]):
            ...
            continue
        dst = os.path.join(dest_project, *entry["rel_path"].split("/"))
        if os.path.isfile(dst):
            if _xfer_same_file(entry, dst, sha_cache):
                result["identical"] += 1
            else:
                result["conflicts"].append((entry, dst))
            continue
        # Don't bring over an OLDER work scene than the destination already has.
        if (
            item["kind"] in ("asset", "shot") and entry.get("task") and entry.get("version") is not None
            and entry.get("category") in ("work", "thumbnail")
        ):
            key = (item["kind"], item.get("asset_type"), item["name"], entry["task"])
            if key not in latest_cache:
                filename, _path = _xfer_latest_work(dest_project, item, entry["task"]) if exists else (None, None)
                latest_cache[key] = _file_xfer_version_of(filename) if filename else None
            dest_version = latest_cache[key]
            if dest_version is not None and dest_version > entry["version"]:
                result["dest_newer"] += 1
                continue
        result["copy"].append((entry, dst))

    if not exists and item_dir is not None:
        status = "new"
    ...
    parts = []
    if status == "new":
        parts.append(f"{len(item['tasks'])} task folder(s)" if item["tasks"] else "folder")
    elif missing_tasks:
        parts.append("add " + ", ".join(_file_xfer_task_label(t) for t in missing_tasks))
    if result["copy"]:
        parts.append(f"{len(result['copy'])} file(s) to copy")
    if result["conflicts"]:
        parts.append(f"{len(result['conflicts'])} different file(s) at the same path")
    if result["dest_newer"]:
        parts.append(f"{result['dest_newer']} skipped (destination has a newer version)")
    if result["source_missing"]:
        parts.append(f"{result['source_missing']} listed but missing from the package")
    if status == "uptodate" and not parts:
...
def _xfer_receive(panel, source, dest_project, rows, apply_settings, switch_project):
    """
    rows: [(item, compare_result)] for the checked items. Builds folders,
    copies files (asking on conflicts), optionally applies settings and
    switches project. Returns True if anything was done.
    """
    dest_is_project = _xfer_looks_like_project(dest_project)
    dest_name = os.path.basename(os.path.normpath(dest_project))
    folder_count = sum(1 for item, r in rows if item["kind"] != "deps" and (r["status"] == "new" or r["missing_tasks"]))
    copy_count = sum(len(r["copy"]) for _i, r in rows)
    conflict_count = sum(len(r["conflicts"]) for _i, r in rows)

    message = f"Receive into {'NEW project ' if not dest_is_project else ''}{dest_name}\n{dest_project}\n\n"
    message += f"  {len(rows)} item(s) checked\n  {folder_count} with folders to build\n  {copy_count} file(s) to copy"
    if conflict_count:
        message += f"\n  {conflict_count} conflict(s) — you'll be asked about each"
    if apply_settings:
        message += "\n  Apply the package's project settings"
    if switch_project:
        message += "\n  Switch to this project afterwards"
    if cmds.confirmDialog(
        title=PROJECT_XFER_PANEL_TITLE, message=message, button=["Receive", "Cancel"],
        defaultButton="Receive", cancelButton="Cancel", dismissString="Cancel",
    ) != "Receive":
        return False

    previous_project = get_current_project(warn_if_missing=False)
    if not dest_is_project:
        build_project_skeleton(dest_project)  # also sets it current — restored below unless switching

    # Folders first (idempotent builders — nothing existing is touched).
    built = []
    for item, _r in rows:
        if item["kind"] == "deps":
            continue
        ...
        try:
            if item["kind"] == "asset":
                if item["tasks"]:
                    build_asset_task_structure(item_dir, tasks=list(item["tasks"]))
                else:
                    os.makedirs(item_dir, exist_ok=True)
            else:
                os.makedirs(item_dir, exist_ok=True)
                if item["tasks"]:
                    build_shot_task_structure(item_dir, tasks=list(item["tasks"]))
            built.append(_xfer_item_label(item))
        except Exception as e:
            cmds.warning(f"Project Xfer: could not build folders for {_xfer_item_label(item)} ({e}).")

    # Conflicts: ask before any copying starts.
    to_copy, skipped = [], []
    for _item, r in rows:
        to_copy.extend(r["copy"])
    blanket = None
    for entry, dst in [c for _i, r in rows for c in r["conflicts"]]:
        choice = blanket
        if choice is None:
            choice = cmds.confirmDialog(
                title=f"{PROJECT_XFER_PANEL_TITLE} — File Exists",
                message=f"A different file already exists at:\n\n{entry['rel_path']}\n\nOverwrite it?",
                button=["Overwrite", "Skip", "Overwrite All", "Skip All"],
                defaultButton="Skip", cancelButton="Skip", dismissString="Skip",
            )
            if choice in ("Overwrite All", "Skip All"):
    ...
    was_cancelled = False
    if to_copy:
        total = sum(os.path.getsize(entry["src"]) for entry, _d in to_copy)
        progress = _FileXferProgress("Copying files...", total, parent=panel)
        progress.dialog.setWindowTitle(PROJECT_XFER_PANEL_TITLE)
        try:
            for i, (entry, dst) in enumerate(to_copy, 1):
                progress.set_label(f"Copying {i} of {len(to_copy)}\n{os.path.basename(dst)}")
                try:
                    sha = _file_xfer_copy(entry["src"], dst, progress.add)
                except Exception as e:
                    failed.append(f"{entry['rel_path']}: {e}")
                    continue
                if sha is None:
    ...
    if apply_settings and source.get("settings"):
        _apply_pipeline_package_settings({"settings": source["settings"]})

    if switch_project:
        switch_to_project(dest_project)
    elif not dest_is_project and previous_project and os.path.isdir(previous_project):
        # build_project_skeleton made the new project current; put it back.
        cmds.optionVar(stringValue=(CURRENT_PROJECT_OPTVAR, previous_project))
        cmds.savePrefs(general=True)
        cmds.evalDeferred(build_menu)

    summary = f"Received into {dest_name}:\n\n  Folders built/checked: {len(built)}\n  Files copied: {len(copied)}"
    if skipped:
        summary += f"\n  Skipped by you: {len(skipped)}"
    if apply_settings and source.get("settings"):
    ...
    if was_cancelled:
        summary += "\n\nCancelled part-way — files copied before cancelling were kept."
    if bad_checksum:
        summary += "\n\nChecksum MISMATCH (copy may be damaged — re-copy the package):\n  " + "\n  ".join(bad_checksum[:15])
    if failed:
        summary += "\n\nFailed:\n  " + "\n  ".join(failed[:15])
    if source.get("external"):
        summary += f"\n\n{len(source['external'])} dependency file(s) lived outside the source project and were not included."
    summary += "\n\nNothing at the destination was deleted."
    cmds.confirmDialog(title=PROJECT_XFER_PANEL_TITLE, message=summary, button=["OK"])
    return True
...
def _project_xfer_stylesheet():
    t = AM_TOKENS
    p = PROJECT_XFER_PANEL_OBJECT_NAME
    return _file_xfer_stylesheet(p) + f"""
        QWidget#{p} QFrame#pxBar {{
            background: {t['bg_section_header']};
            border-bottom: 1px solid {t['border']};
        }}
        QWidget#{p} QLabel#pxBarLabel {{ color: {t['text_muted']}; font-size: 11.5px; font-weight: 600; }}
        QWidget#{p} QLabel#pxSourceLabel {{ color: {t['text_primary']}; font-size: 12.5px; }}
        QWidget#{p} QTreeWidget {{
            background: {t['bg_dark_column']};
            color: {t['text_primary']};
            border: none;
            font-size: 12.5px;
        }}
        QWidget#{p} QTreeWidget::item {{ padding: 3px 2px; }}
        QWidget#{p} QHeaderView::section {{
            background: {t['bg_section_header']};
            color: {t['text_muted']};
            border: none;
            border-bottom: 1px solid {t['border']};
        ...
        QWidget#{p} QComboBox QAbstractItemView {{
            background: {t['bg_dark_column']};
            color: {t['text_primary']};
            selection-background-color: {t['blue_row_bg']};
        }}
    """


class _XferBrowseRow(QtWidgets.QFrame):
    """Browse row with BOTH a "+" (stage) and a click-through (drill in) — Send's asset/shot rows."""

    clicked = QtCore.Signal()
    add_clicked = QtCore.Signal()

    def __init__(self, name, sub, has_add=True, has_chevron=True, parent=None):
        super().__init__(parent)
        self.setCursor(QtCore.Qt.PointingHandCursor)
        layout = QtWidgets.QHBoxLayout(self)
        layout.setContentsMargins(8, 7, 8, 7)
        ...
        if has_add:
            ...
            add_btn.setCursor(QtCore.Qt.PointingHandCursor)
            add_btn.setFixedSize(16, 16)
            add_btn.setToolTip("Stage the whole thing")
            add_btn.clicked.connect(self.add_clicked.emit)
            layout.addWidget(add_btn)
        if has_chevron:
            chevron = QtWidgets.QLabel("›")
...
def _xfer_link_label(text, callback):
    label = QtWidgets.QLabel(text)
    label.setObjectName("amBackButton")
    label.setCursor(QtCore.Qt.PointingHandCursor)
    label.mousePressEvent = lambda e: callback()
    return label


def _xfer_footer_button(text, primary, callback, tooltip=None):
    btn = QtWidgets.QPushButton(text)
    btn.setProperty("class", "amFooterUpdate" if primary else "amFooterCancel")
    btn.setCursor(QtCore.Qt.PointingHandCursor)
    if tooltip:
...
class ProjectXferPanel(QtWidgets.QWidget):
    """3.15.0 — see the "Project Xfer" module comment. Plain floating QWidget (not dockable)."""

    def __init__(self, project_path, parent=None):
        super().__init__(parent)
        self.project_path = project_path  # current project (may be None — Receive still works)
        # Send state
        self.root_tab = "Assets"
        self.asset_type = None
        self.item_key = None  # drilled-into (kind, asset_type, name)
        self.staged = {}  # (kind, asset_type, name) -> {kind, name, asset_type, tasks: None (whole) | [tasks]}
        # Receive state
        self.source = None
        self.dest_path = None
        self.compare_rows = []  # [(item, result, tree_item)]
        self._sha_cache = {}

        self.setObjectName(PROJECT_XFER_PANEL_OBJECT_NAME)
        self.setWindowTitle(PROJECT_XFER_PANEL_TITLE)
        self.setWindowFlags(QtCore.Qt.Window)
        self.setStyleSheet(_project_xfer_stylesheet())
        self._build_ui()
        self._set_mode("Send" if project_path else "Receive")
        self.resize(860, 560)
    ...
    def _build_ui(self):
        ...
        header_layout = QtWidgets.QHBoxLayout(header)
        header_layout.setContentsMargins(12, 8, 12, 8)
        project_name = os.path.basename(os.path.normpath(self.project_path)) if self.project_path else "(none)"
        title = QtWidgets.QLabel(f"{PROJECT_XFER_PANEL_TITLE} — Project: {project_name}")
        title.setObjectName("amHeaderTitle")
        header_layout.addWidget(title)
        header_layout.addStretch(1)
        self.send_label = QtWidgets.QLabel("Send")
        header_layout.addWidget(self.send_label)
        self.mode_toggle = _ToggleSwitch()
        self.mode_toggle.setChecked(False)
        self.mode_toggle.toggled.connect(lambda checked: self._set_mode("Receive" if checked else "Send"))
        header_layout.addWidget(self.mode_toggle)
        self.receive_label = QtWidgets.QLabel("Receive")
        header_layout.addWidget(self.receive_label)
        outer.addWidget(header)
    ...
    @staticmethod
    def _repolish(label, object_name):
    ...
    def _set_mode(self, mode):
        ...
        if self.mode_toggle.isChecked() != receive:
            self.mode_toggle.blockSignals(True)
            self.mode_toggle.setChecked(receive)
            self.mode_toggle.blockSignals(False)
        self._repolish(self.send_label, "amToggleLabel" if receive else "amToggleLabelActive")
        self._repolish(self.receive_label, "amToggleLabelActive" if receive else "amToggleLabel")
        self.pages.setCurrentWidget(self.receive_page if receive else self.send_page)
        if not receive:
            self._refresh_browse()
    ...
    def _build_send_page(self):
        ...
        browse_col.setObjectName("amBrowseList")
        browse_col.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        browse_col.setFixedWidth(300)
        browse_layout = QtWidgets.QVBoxLayout(browse_col)
        browse_layout.setContentsMargins(0, 0, 0, 0)
        browse_layout.setSpacing(0)

        toggle_row = QtWidgets.QWidget()
        toggle_row.setObjectName("amBrowseHeaderRow")
        toggle_row.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        toggle_layout = QtWidgets.QHBoxLayout(toggle_row)
        toggle_layout.setContentsMargins(10, 6, 10, 6)
        toggle_layout.setSpacing(6)
        toggle_layout.addStretch(1)
        self.assets_label = QtWidgets.QLabel("Assets")
        toggle_layout.addWidget(self.assets_label)
        self.root_toggle = _ToggleSwitch()
        self.root_toggle.setChecked(False)
        self.root_toggle.toggled.connect(self._on_root_toggle)
        toggle_layout.addWidget(self.root_toggle)
        self.shots_label = QtWidgets.QLabel("Shots")
        toggle_layout.addWidget(self.shots_label)
        toggle_layout.addStretch(1)
        browse_layout.addWidget(toggle_row)
        ...
        header_row_layout.setContentsMargins(10, 4, 10, 4)
        header_row_layout.setSpacing(6)
        self.back_button = _xfer_link_label("‹ Back", self._on_back)
        header_row_layout.addWidget(self.back_button)
        self.browse_header = QtWidgets.QLabel("Assets")
        self.browse_header.setObjectName("amBrowseHeaderLabel")
        header_row_layout.addWidget(self.browse_header, 1)
        ...
        staged_header_layout = QtWidgets.QHBoxLayout(staged_header_row)
        staged_header_layout.setContentsMargins(10, 4, 10, 4)
        staged_header_layout.setSpacing(10)
        self.staged_header = QtWidgets.QLabel("To Send — 0")
        self.staged_header.setObjectName("amBrowseHeaderLabel")
        staged_header_layout.addWidget(self.staged_header, 1)
        staged_header_layout.addWidget(_xfer_link_label("+ Full Project", self._on_full_project))
        staged_header_layout.addWidget(_xfer_link_label("+ All Assets", self._on_all_assets))
        staged_header_layout.addWidget(_xfer_link_label("+ All Shots", self._on_all_shots))
        staged_header_layout.addWidget(_xfer_link_label("Clear", self._clear_staged))
        staged_layout.addWidget(staged_header_row)
        staged_scroll = QtWidgets.QScrollArea()
        staged_scroll.setObjectName("amSceneScroll")
        staged_scroll.setWidgetResizable(True)
        ...
        footer_layout.setContentsMargins(10, 10, 10, 10)
        footer_layout.setSpacing(12)
        self.files_check = QtWidgets.QCheckBox("Scene Files")
        self.files_check.setChecked(True)
        self.files_check.setToolTip(
            "Include each staged task's latest work scene (and its thumbnail).\n"
            "Off = folder structure + project settings only."
        )
        self.files_check.toggled.connect(self._on_files_toggled)
        footer_layout.addWidget(self.files_check)
        self.publishes_check = QtWidgets.QCheckBox("Include Publishes")
        self.publishes_check.setToolTip("Also include each staged asset task's latest publish (output/geo or output/rig).")
        footer_layout.addWidget(self.publishes_check)
        self.deps_check = QtWidgets.QCheckBox("Include Dependencies")
        self.deps_check.setToolTip(
            "Also include the files each scene points at (references, textures, alembic caches)\n"
            "that live inside this project."
        )
        footer_layout.addWidget(self.deps_check)
        footer_layout.addStretch(1)
        footer_layout.addWidget(_xfer_footer_button("Send", True, self._on_send))
        footer_layout.addWidget(_xfer_footer_button("Close", False, self.close))
        layout.addWidget(footer)
        return page

    def _on_files_toggled(self, checked):
        self.publishes_check.setEnabled(checked)
        self.deps_check.setEnabled(checked)

    def _update_root_labels(self):
        shots = self.root_tab == "Shots"
        self._repolish(self.assets_label, "amToggleLabel" if shots else "amToggleLabelActive")
        self._repolish(self.shots_label, "amToggleLabelActive" if shots else "amToggleLabel")
    ...
    def _add_browse_widget(self, widget):
        self.browse_list_layout.insertWidget(self.browse_list_layout.count() - 1, widget)

    def _add_browse_note(self, text):
        label = QtWidgets.QLabel(text)
        label.setWordWrap(True)
        label.setStyleSheet(f"color:{AM_TOKENS['text_muted']}; font-size:11.5px; padding:8px;")
        self._add_browse_widget(label)
    ...
    def _refresh_browse(self):
        ...
        if not self.project_path:
            self.back_button.setVisible(False)
            self.browse_header.setText("")
            self._add_browse_note("No current project is set — switch to a project to send from it.")
            return

        if self.item_key is not None:
            kind, asset_type, name = self.item_key
            item = {"kind": kind, "asset_type": asset_type or None, "name": name}
            self.back_button.setVisible(True)
            self.browse_header.setText(f"{asset_type.capitalize()} › {name}" if kind == "asset" else name)
            item_dir = _xfer_item_dir(self.project_path, item)
            tasks = _xfer_disk_tasks(item_dir, kind)
            if not tasks:
                self._add_browse_note("No task folders here yet.")
            scene_tasks = _xfer_scene_tasks(kind)
            for task in tasks:
                filename = None
                if task in scene_tasks:
                    filename, _path = _xfer_latest_work(self.project_path, item, task)
                sub = filename or ("folder only (no Maya scenes)" if task not in scene_tasks else "folder only — no saved scene yet")
                row = _AssetManagerBrowseRow(_file_xfer_task_label(task), sub, has_add=True)
                row.add_clicked.connect(lambda t=task, i=item: self._stage(i, [t]))
                self._add_browse_widget(row)
            return
        ...
        if self.root_tab == "Shots":
            ...
            shots = list_existing_shots(self.project_path)
            for shot in shots:
                item = {"kind": "shot", "asset_type": None, "name": shot}
                count = len(_xfer_disk_tasks(_xfer_item_dir(self.project_path, item), "shot"))
                row = _XferBrowseRow(shot, f"{count} task folder{'s' if count != 1 else ''}")
                row.add_clicked.connect(lambda i=item: self._stage(i, None))
                row.clicked.connect(lambda s=shot: self._open_item(("shot", "", s)))
                self._add_browse_widget(row)
            if not shots:
                self._add_browse_note("No shots in this project yet.")
            return
        ...
        self.browse_header.setText(self.asset_type.capitalize())
        assets = list_all_assets(self.project_path, type_name=self.asset_type)
        for asset_name, _asset_dir in assets:
            item = {"kind": "asset", "asset_type": self.asset_type, "name": asset_name}
            count = len(_xfer_disk_tasks(_xfer_item_dir(self.project_path, item), "asset"))
            row = _XferBrowseRow(asset_name, f"{count} task folder{'s' if count != 1 else ''}")
            row.add_clicked.connect(lambda i=item: self._stage(i, None))
            row.clicked.connect(lambda n=asset_name, t=self.asset_type: self._open_item(("asset", t, n)))
            self._add_browse_widget(row)
        if not assets:
            self._add_browse_note(f"No {self.asset_type} assets yet.")
    ...
    def _open_item(self, key):
        self.item_key = key
        self._refresh_browse()

    # ---- staging ----

    def _stage(self, item, tasks):
        """tasks None = the whole asset/shot; otherwise add those tasks to it."""
        key = (item["kind"], item.get("asset_type") or "", item["name"])
        entry = self.staged.get(key)
        if entry is None:
            entry = {"kind": item["kind"], "name": item["name"], "asset_type": item.get("asset_type"), "tasks": []}
            self.staged[key] = entry
        if tasks is None:
            entry["tasks"] = None
        elif entry["tasks"] is not None:
            for task in tasks:
                if task not in entry["tasks"]:
                    entry["tasks"].append(task)
        self._refresh_staged()

    def _stage_whole(self, items):
        for item in items:
            key = (item["kind"], item.get("asset_type") or "", item["name"])
            self.staged[key] = {"kind": item["kind"], "name": item["name"], "asset_type": item.get("asset_type"), "tasks": None}
        self._refresh_staged()

    def _all_asset_items(self):
        return [
            {"kind": "asset", "asset_type": os.path.basename(os.path.dirname(asset_dir)), "name": name}
            for name, asset_dir in list_all_assets(self.project_path)
        ]

    def _all_shot_items(self):
        return [{"kind": "shot", "asset_type": None, "name": s} for s in list_existing_shots(self.project_path)]

    def _on_full_project(self):
        if self.project_path:
            self._stage_whole(self._all_asset_items() + self._all_shot_items())

    def _on_all_assets(self):
        if self.project_path:
            self._stage_whole(self._all_asset_items())

    def _on_all_shots(self):
        if self.project_path:
            self._stage_whole(self._all_shot_items())
    ...
    def _refresh_staged(self):
        while self.staged_list_layout.count() > 1:
            ...
            if widget:
                widget.deleteLater()
        self.staged_header.setText(f"To Send — {len(self.staged)}")
        for key, entry in self.staged.items():
            kind_label = "Asset" if entry["kind"] == "asset" else "Shot"
            if entry["tasks"] is None:
                sub = f"{kind_label} — all tasks"
            else:
                sub = f"{kind_label} — " + ", ".join(_file_xfer_task_label(t) for t in entry["tasks"])
            row = _PipelineStagedRow(_xfer_item_label(entry), sub)
            row.remove_clicked.connect(lambda k=key: self._unstage(k))
            self.staged_list_layout.insertWidget(self.staged_list_layout.count() - 1, row)

    def _on_send(self):
        if not self.project_path:
            cmds.warning("Project Xfer: no current project to send from.")
            return
        if not self.staged:
            cmds.warning("Project Xfer: stage at least one asset or shot first.")
            return
        include_files = self.files_check.isChecked()
        if _xfer_send(
            self, self.project_path, self.staged, include_files,
            include_files and self.publishes_check.isChecked(),
            include_files and self.deps_check.isChecked(),
        ):
            self._clear_staged()
    ...
    def _build_receive_page(self):
        ...
        from_bar = QtWidgets.QFrame()
        from_bar.setObjectName("pxBar")
        from_layout = QtWidgets.QHBoxLayout(from_bar)
        from_layout.setContentsMargins(12, 8, 12, 8)
        from_layout.setSpacing(8)
        from_label = QtWidgets.QLabel("FROM")
        from_label.setObjectName("pxBarLabel")
        from_label.setFixedWidth(40)
        from_layout.addWidget(from_label)
        self.source_label = QtWidgets.QLabel("Pick a package or a project folder to receive from.")
        self.source_label.setObjectName("pxSourceLabel")
        self.source_label.setSizePolicy(QtWidgets.QSizePolicy.Ignored, QtWidgets.QSizePolicy.Preferred)
        from_layout.addWidget(self.source_label, 1)
        from_layout.addWidget(_xfer_footer_button(
            "Package…", False, self._on_pick_package,
            "A Project Xfer folder (xfer_manifest.json), an older File Xfer Assist folder,\n"
            "an Export Pipeline .json, or a Project Sync io/manifest.json.",
        ))
        from_layout.addWidget(_xfer_footer_button(
            "Project Folder…", False, self._on_pick_project,
            "Compare straight against another copy of a project (e.g. your other drive).",
        ))
        layout.addWidget(from_bar)

        into_bar = QtWidgets.QFrame()
        into_bar.setObjectName("pxBar")
        into_layout = QtWidgets.QHBoxLayout(into_bar)
        into_layout.setContentsMargins(12, 8, 12, 8)
        into_layout.setSpacing(8)
        into_label = QtWidgets.QLabel("INTO")
        into_label.setObjectName("pxBarLabel")
        into_label.setFixedWidth(40)
        into_layout.addWidget(into_label)
        self.dest_combo = QtWidgets.QComboBox()
        self.dest_combo.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Fixed)
        self.dest_combo.activated.connect(self._on_dest_activated)
        into_layout.addWidget(self.dest_combo, 1)
        layout.addWidget(into_bar)

        list_bar = QtWidgets.QWidget()
        list_bar.setObjectName("amBrowseHeaderRow")
        list_bar.setAttribute(QtCore.Qt.WA_StyledBackground, True)
        list_bar_layout = QtWidgets.QHBoxLayout(list_bar)
        list_bar_layout.setContentsMargins(10, 4, 10, 4)
        list_bar_layout.setSpacing(10)
        self.compare_header = QtWidgets.QLabel("Compare")
        self.compare_header.setObjectName("amBrowseHeaderLabel")
        list_bar_layout.addWidget(self.compare_header, 1)
        list_bar_layout.addWidget(_xfer_link_label("Check Changed", lambda: self._check_rows("changed")))
        list_bar_layout.addWidget(_xfer_link_label("All", lambda: self._check_rows("all")))
        list_bar_layout.addWidget(_xfer_link_label("None", lambda: self._check_rows("none")))
        layout.addWidget(list_bar)

        self.tree = QtWidgets.QTreeWidget()
        self.tree.setColumnCount(3)
        self.tree.setHeaderLabels(["Item", "Status", "Details"])
        ...
        footer_layout.setContentsMargins(10, 10, 10, 10)
        footer_layout.setSpacing(12)
        self.copy_files_check = QtWidgets.QCheckBox("Copy Scene Files")
        self.copy_files_check.setChecked(False)
        self.copy_files_check.setToolTip(
            "Off: rebuild missing folders only.\n"
            "On: also copy each task's latest work scene when the destination\n"
            "doesn't already have that version or newer."
        )
        self.copy_files_check.toggled.connect(self._on_copy_files_toggled)
        footer_layout.addWidget(self.copy_files_check)
        self.settings_check = QtWidgets.QCheckBox("Apply Project Settings")
        self.settings_check.setToolTip("Restore the package's shot prefix, resolution, frame range and frame rate.")
        footer_layout.addWidget(self.settings_check)
        self.switch_check = QtWidgets.QCheckBox("Switch to Project")
        self.switch_check.setChecked(True)
        self.switch_check.setToolTip("Make the destination the current project when done.")
        footer_layout.addWidget(self.switch_check)
        footer_layout.addStretch(1)
        footer_layout.addWidget(_xfer_footer_button("Receive", True, self._on_receive))
        footer_layout.addWidget(_xfer_footer_button("Close", False, self.close))
        layout.addWidget(footer)

        self._update_receive_options()
        self._rebuild_dest_combo()
        self._show_tree_message("Pick a package or a project folder to receive from.")
        return page

    def _show_tree_message(self, text):
        ...
        self.compare_rows = []
        self.compare_header.setText("Compare")

    def _update_receive_options(self):
        is_project = bool(self.source) and self.source["type"] == "project"
        self.copy_files_check.setVisible(is_project)
        has_settings = bool(self.source) and bool(self.source.get("settings"))
        self.settings_check.setEnabled(has_settings)
        if not has_settings:
            self.settings_check.setChecked(False)

    # ---- source ----

    def _on_pick_package(self):
        start = os.path.join(self.project_path, "io", PROJECT_XFER_SUBDIR) if self.project_path else ""
        if start and not os.path.isdir(start):
            start = os.path.join(self.project_path, "io")
        kwargs = {}
        if start and os.path.isdir(start):
            kwargs["startingDirectory"] = start
        result = cmds.fileDialog2(
            fileMode=1,
            caption="Select a package — xfer_manifest.json inside a Send folder, or an older .json",
            fileFilter="Packages (*.json)",
            **kwargs
        )
        if not result:
            return
        source = _xfer_load_package(result[0])
        if not source:
            return
        self._set_source(source)

    def _on_pick_project(self):
        result = cmds.fileDialog2(fileMode=3, caption="Select a project folder to receive from", okCaption="Select")
        if not result:
            return
        path = os.path.normpath(result[0])
        if not _xfer_looks_like_project(path):
            cmds.confirmDialog(
                title=PROJECT_XFER_PANEL_TITLE,
                message=f"That folder doesn't look like a project (no assets/ or shots/ inside):\n{path}",
                button=["OK"],
            )
            return
        self._set_source(_xfer_load_project_source(path, self.copy_files_check.isChecked()))

    def _set_source(self, source):
        self.source = source
        self.dest_path = None  # re-pick the default destination for this source
        self.source_label.setText(source["label"])
        self.source_label.setToolTip(source["path"])
        self._update_receive_options()
        self._rebuild_dest_combo()
        self._recompare()

    def _on_copy_files_toggled(self, checked):
        if self.source and self.source["type"] == "project":
            self.source = _xfer_load_project_source(self.source["path"], checked)
            self._recompare()

    # ---- destination ----

    def _rebuild_dest_combo(self):
        """Entries: current project, the same-named project in the projects folder, other folder…"""
        self.dest_combo.blockSignals(True)
        self.dest_combo.clear()
        current = self.project_path
        source_name = self.source.get("project_name") if self.source else None
        root = get_projects_root()
        entries = []
        if current:
            entries.append((f"Current project — {os.path.basename(os.path.normpath(current))}   ({current})", current))
        if source_name:
            same = os.path.normpath(os.path.join(root, source_name)) if root else None
            source_folder = self.source.get("project_path")
            if same and source_folder and os.path.normcase(same) == os.path.normcase(os.path.normpath(source_folder)):
                same = None  # that's the source itself
                root_known = True
            else:
                root_known = bool(root)
            if same and (not current or os.path.normcase(same) != os.path.normcase(os.path.normpath(current))):
                state = "existing" if _xfer_looks_like_project(same) else "NEW"
                entries.append((f"{source_name} in the projects folder ({state})   ({same})", same))
            elif not root_known:
                entries.append((f"New project '{source_name}' — choose the projects folder…", "__root__"))
        if self.dest_path and all(
            os.path.normcase(os.path.normpath(p)) != os.path.normcase(os.path.normpath(self.dest_path))
            for _l, p in entries if p != "__root__"
        ):
            state = "existing" if _xfer_looks_like_project(self.dest_path) else "NEW"
            entries.append((f"{os.path.basename(os.path.normpath(self.dest_path))} ({state})   ({self.dest_path})", self.dest_path))
        entries.append(("Other folder…", "__browse__"))
        for label, data in entries:
            self.dest_combo.addItem(label, data)

        # Default: keep the previous pick; else the project matching the
        # package's name; else the current project.
        index = -1
        if self.dest_path:
            index = self._combo_index_for(self.dest_path)
        if index < 0 and source_name:
            for i in range(self.dest_combo.count()):
                data = self.dest_combo.itemData(i)
                if data not in ("__browse__", "__root__") and os.path.basename(os.path.normpath(data)) == source_name:
                    index = i
                    break
        if index < 0:
            index = 0 if self.dest_combo.itemData(0) not in ("__browse__", "__root__") else -1
        if index >= 0:
            self.dest_combo.setCurrentIndex(index)
            self.dest_path = self.dest_combo.itemData(index)
        else:
            self.dest_path = None
            self.dest_combo.setCurrentIndex(-1)
        self.dest_combo.blockSignals(False)
        self._update_settings_default()

    def _combo_index_for(self, path):
        for i in range(self.dest_combo.count()):
            data = self.dest_combo.itemData(i)
            if data in ("__browse__", "__root__"):
                continue
            if os.path.normcase(os.path.normpath(data)) == os.path.normcase(os.path.normpath(path)):
                return i
        return -1

    def _on_dest_activated(self, index):
        data = self.dest_combo.itemData(index)
        if data == "__browse__":
            result = cmds.fileDialog2(fileMode=3, caption="Select the destination project folder", okCaption="Select")
            if result:
                self.dest_path = os.path.normpath(result[0])
            self._rebuild_dest_combo()
        elif data == "__root__":
            root = _pipeline_import_destination_root()
            if root and self.source and self.source.get("project_name"):
                self.dest_path = os.path.normpath(os.path.join(root, self.source["project_name"]))
            self._rebuild_dest_combo()
        else:
            self.dest_path = data
            self._update_settings_default()
    ...
    def _update_settings_default(self):
        """Settings are global, so only pre-check them when receiving into a brand-new project."""
        if self.settings_check.isEnabled():
            self.settings_check.setChecked(bool(self.dest_path) and not _xfer_looks_like_project(self.dest_path))
    ...
    def _recompare(self):
        ...
        if not self.dest_path:
            self._show_tree_message("Pick where to receive into.")
            return
        if self.source["type"] == "project" and (
            os.path.normcase(os.path.normpath(self.source["path"])) == os.path.normcase(os.path.normpath(self.dest_path))
        ):
            self._show_tree_message("Source and destination are the same folder — pick a different one.")
            return

        dest_is_project = _xfer_looks_like_project(self.dest_path)
        latest_cache = {}
        QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
        try:
            results = [
                (item, _xfer_compare_item(item, self.dest_path, dest_is_project, self._sha_cache, latest_cache))
                for item in _xfer_sorted_items(self.source["items"])
            ]
        finally:
            QtWidgets.QApplication.restoreOverrideCursor()
        ...
        self.compare_rows = []
        groups = {}
        group_titles = {"asset": "Assets", "shot": "Shots", "deps": "Dependencies"}
        for item, result in results:
            kind = item["kind"]
            if kind not in groups:
                group = QtWidgets.QTreeWidgetItem([group_titles.get(kind, kind), "", ""])
                group.setFlags(group.flags() | QtCore.Qt.ItemIsUserCheckable | QtCore.Qt.ItemIsAutoTristate)
                font = group.font(0)
                font.setBold(True)
                group.setFont(0, font)
                self.tree.addTopLevelItem(group)
                groups[kind] = group
            label = "Files not in any asset/shot" if kind == "deps" else _xfer_item_label(item)
            status_label, tint = XFER_STATUS[result["status"]]
            row = QtWidgets.QTreeWidgetItem([label, status_label, result["detail"]])
            row.setFlags(row.flags() | QtCore.Qt.ItemIsUserCheckable)
            changed = result["status"] != "uptodate"
            row.setCheckState(0, QtCore.Qt.Checked if changed else QtCore.Qt.Unchecked)
            if tint:
                for col in range(3):
                    row.setBackground(col, QtGui.QBrush(QtGui.QColor(tint)))
            else:
                for col in range(3):
                    row.setForeground(col, QtGui.QBrush(QtGui.QColor(AM_TOKENS["text_muted"])))
            row.setToolTip(2, result["detail"])
            groups[kind].addChild(row)
            self.compare_rows.append((item, result, row))
        for group in groups.values():
            group.setExpanded(True)
            group.setText(0, f"{group.text(0)}  ({group.childCount()})")
        self.tree.blockSignals(False)
        self._update_compare_header()
        if not results:
            self._show_tree_message("The source has nothing in it.")

    def _on_tree_item_changed(self, _item, _column):
        self._update_compare_header()

    def _update_compare_header(self):
        changed = sum(1 for _i, r, _row in self.compare_rows if r["status"] != "uptodate")
        checked = sum(1 for _i, _r, row in self.compare_rows if row.checkState(0) == QtCore.Qt.Checked)
        self.compare_header.setText(
            f"Compare — {len(self.compare_rows)} item(s), {changed} with changes, {checked} checked"
        )

    def _check_rows(self, which):
        self.tree.blockSignals(True)
        for _item, result, row in self.compare_rows:
            if which == "all":
                state = True
            elif which == "none":
                state = False
            else:
                state = result["status"] != "uptodate"
            row.setCheckState(0, QtCore.Qt.Checked if state else QtCore.Qt.Unchecked)
        self.tree.blockSignals(False)
    ...
    def _on_receive(self):
        if not self.source:
            cmds.warning("Project Xfer: pick a package or a project folder first.")
            return
        if not self.dest_path:
            cmds.warning("Project Xfer: pick where to receive into.")
            return
        rows = [(item, result) for item, result, row in self.compare_rows if row.checkState(0) == QtCore.Qt.Checked]
        if not rows:
            cmds.warning("Project Xfer: nothing is checked.")
            return
        if _xfer_receive(
            self, self.source, self.dest_path, rows,
            self.settings_check.isEnabled() and self.settings_check.isChecked(),
            self.switch_check.isChecked(),
        ):
            if self.source["type"] == "project":
                self.source = _xfer_load_project_source(self.source["path"], self.copy_files_check.isChecked())
            self._sha_cache = {}
            self._rebuild_dest_combo()
            self._recompare()


def show_project_xfer_panel():
    """Menu entry point — TP_pipe > Project Utilities > Project Xfer."""
    global _project_xfer_panel_instance
    project_path = get_current_project(warn_if_missing=False)
    if _project_xfer_panel_instance is not None:
        try:
            _project_xfer_panel_instance.close()
            _project_xfer_panel_instance.deleteLater()
        except Exception:
            pass
    panel = ProjectXferPanel(project_path)
    _project_xfer_panel_instance = panel
    _pipeline_panel_show(panel)
    return panel
