...
def _get_aov_node(interface, aov_name):
    """
    2.36.0: the mtoa build shipping with Maya 2027 added a required
    second positional arg to AOVInterface.getAOVNode() (aovName,
    layerName) — older mtoa only takes aovName. Try the new signature
    first and fall back to the old one on TypeError, so this works
    unmodified on either Arnold/mtoa version rather than assuming which
    one is installed. "defaultRenderLayer" (Maya's always-present master
    layer) is used for the existence check regardless of whatever render
    layer happens to be current in the outliner, since AOV creation here
    is meant to be a global check, independent from the per-layer AOV
    overrides handled separately by _set_layer_aov_enabled.
    """
    try:
        return interface.getAOVNode(aov_name, "defaultRenderLayer")
    except TypeError:
        return interface.getAOVNode(aov_name)
...
def create_aovs(beauty_selected, utility_selected, rgba_selected):
    ...
    for aov_name in beauty_selected + utility_selected:
        try:
            if not _get_aov_node(interface, aov_name):
    ...
    if rgba_selected:
        try:
            if not _get_aov_node(interface, "RGBA"):
