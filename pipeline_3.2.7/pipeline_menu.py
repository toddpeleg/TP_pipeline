...
def open_panel_shot_scene_files(project_path, shot_name, task_name):
    """Every work-in-progress scene file for one shot+task, newest first, as (filename, full_path) pairs."""
    ...
    return _list_versioned_scene_files(scenes_dir, f"{shot_name}_{task_name}")
...
OPEN_SCENE_WINDOW_SIZE = (980, 560)
...
def _open_scene_stylesheet():
    ...
    return f"""
        ...
        QWidget#{OPEN_SCENE_PANEL_OBJECT_NAME} QLabel#openSceneProjectLabel {{
            color: {t['text_primary']};
            font-weight: bold;
        }}
    """
...
class OpenScenePanel(MayaQWidgetDockableMixin, QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self._build_ui()
        # bookmark_list's fixed height is computed after _build_ui() so the
        # panel's stylesheet (set above, before _build_ui) has already
        # settled on the widgets by the time sizeHintForRow is read.
        self._resize_bookmark_list()
        self._refresh_tree()

    # ---------------- UI construction ----------------
    #
    # 3.0.5 — bookmark box is now a QListWidget (self.bookmark_list) with
    # both "Assets" and "Shots" rows always visible (Todd, after 3.0.3's
    # single toggle button: "i want assets and shots there like before"),
    # switched to a list (from the 3.0.4 pair of QPushButtons) so its
    # selection highlight is drawn by the native style/palette applied
    # in __init__ instead of a hand-picked "active" QSS color.
    ...
    def _build_ui(self):
        ...
        # 3.2.5 — Todd: echo the active project top-left, above the
        # bookmark list, same treatment as Save As's 3.2.4 project label.
        project_label = QtWidgets.QLabel(f"Project: {get_project_menu_label()}")
        project_label.setObjectName("openSceneProjectLabel")
        browse_layout.addWidget(project_label)

        # 3.0.5 — bookmark box: both Assets and Shots visible at once as
        # list rows, click either to switch (no toggle, no dropdown) —
        # Todd: "i want assets and shots there like before". A 2-row,
        # non-scrolling QListWidget rather than buttons so its selected-
        # row look comes from the native style, matching how a real
        # Explorer favorites list actually renders selection.
        self.bookmark_list = QtWidgets.QListWidget()
        self.bookmark_list.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.bookmark_list.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.bookmark_list.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        ...
        for tab_name in ("Assets", "Shots"):
            ...
            item.setData(QtCore.Qt.UserRole, tab_name)
            self.bookmark_list.addItem(item)
        # 3.0.7 — height is set later, in __init__ via _resize_bookmark_list(),
        # AFTER the native style is applied (see the comment there for why).
        self.bookmark_list.currentItemChanged.connect(self._on_bookmark_item_changed)
        browse_layout.addWidget(self.bookmark_list)
        ...
        outer.addWidget(file_col, 1)

    # ---------------- Navigation (browse tree) ----------------

    def _resize_bookmark_list(self):
        """
        Fixed-height the 2-row bookmark list to exactly fit its two rows,
        no scrollbar, no dead space — must run AFTER _apply_open_scene_
        native_look() (see call site in __init__) since the native style
        changes the actual rendered row height beyond what sizeHintForRow
        reports before the style is applied.
        """
        row_height = self.bookmark_list.sizeHintForRow(0)
        if row_height <= 0:
            row_height = self.bookmark_list.fontMetrics().height() + 10  # safe fallback
        frame = self.bookmark_list.frameWidth() * 2
        self.bookmark_list.setFixedHeight(row_height * self.bookmark_list.count() + frame + 4)

    def _sync_bookmark_selection(self):
        for row in range(self.bookmark_list.count()):
            item = self.bookmark_list.item(row)
            if item.data(QtCore.Qt.UserRole) == self.root_tab:
                self.bookmark_list.setCurrentItem(item)
                return

    def _on_bookmark_item_changed(self, current, _previous):
        if current is None:
            return
        tab = current.data(QtCore.Qt.UserRole)
        if tab and tab != self.root_tab:
    ...
    def _refresh_tree(self):
        ...
        if self.root_tab == "Assets":
            ...
            for shot_name in list_existing_shots(self.project_path):
                shot_item = QtWidgets.QTreeWidgetItem([self._branch_label(shot_name)])
                ...
                for task_name in OPEN_SCENE_SHOT_TASKS:
                    ...
                    task_item.setData(0, QtCore.Qt.UserRole, {
                        "kind": "shot_task", "shot": shot_name, "task": task_name,
                    })
    ...
    def _populate_file_table(self, pairs):
        ...
        me = current_pipeline_user()
        rows = []
        for filename, full_path in pairs:
            info = read_scene_comment_info(full_path)
            file_user = info.get("user", "")
            # Current-user-only filtering (per the 3.0.0 design): a scene
            # saved before this feature existed (or outside the pipeline)
            # has no recorded user, and is shown rather than hidden --
            # there's no way to attribute it, so hiding it would just make
            # otherwise-real work invisible.
            if file_user and file_user != me:
                continue
            match = VERSIONED_FILE_PATTERN.match(filename)
            version_label = f"v{match.group(2)}" if match else filename
...
SAVE_AS_WINDOW_SIZE = (980, 620)  # 3.2.0 — same width as Open Scene, taller for the bottom save panel
...
def _save_as_stylesheet():
    ...
    return f"""
        ...
        QWidget#{SAVE_AS_PANEL_OBJECT_NAME} QLabel#saveAsProjectLabel {{
            color: {t['text_primary']};
            font-weight: bold;
        }}
    """
...
class SaveAsPanel(MayaQWidgetDockableMixin, QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        # 3.2.4 — Todd: moved the active-project echo from a top-right
        # header spanning the whole panel (3.2.3) to sit here instead,
        # top-left, directly above the Assets/Shots bookmark list.
        project_label = QtWidgets.QLabel(f"Project: {get_project_menu_label()}")
        project_label.setObjectName("saveAsProjectLabel")
        browse_layout.addWidget(project_label)
    ...
    def _resize_bookmark_list(self):
        row_height = self.bookmark_list.sizeHintForRow(0)
        if row_height <= 0:
            row_height = self.bookmark_list.fontMetrics().height() + 10
        frame = self.bookmark_list.frameWidth() * 2
        self.bookmark_list.setFixedHeight(row_height * self.bookmark_list.count() + frame + 4)
    ...
    def _refresh_tree(self):
        ...
        Shots: Shot > Task (leaf) — identical shape to Open Scene's.
        """
        self.browse_tree.clear()
        if not self.project_path:
            placeholder = QtWidgets.QTreeWidgetItem(["No current project"])
