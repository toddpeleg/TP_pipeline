...
def build_menu():
    ...
    if available_projects:
        ...
        cmds.menuItem(
            label="Set Master Projects Location", command=lambda *a: select_project_root(), parent=project_context_menu
        )
...
def show_create_project_window():
    ...
    location_field = cmds.textField(text="", editable=False)
...
def show_create_aovs_window():
    ...
    cmds.button(label="Apply", width=85, command=on_apply)
    cmds.button(label="Cancel", width=85, command=lambda *a: cmds.deleteUI(window))
    cmds.button(label="Default", width=85, command=on_default)
    cmds.setParent("..")

    cmds.text(label="")  # spacer before the destructive testing button below
    cmds.button(label="Clear AOVs", width=180, command=on_clear_aovs)
...
def asset_task_maya_dir(asset_dir, task_name):
    """
    Path to a task's Maya scenes folder: <asset_dir>/<task_name>/work/maya.
    Every asset task except texture has one (texture is a flat folder with
    no work/maya split) — see build_asset_task_structure.
    """
...
def asset_task_source_dir(asset_dir, task_name):
    ...
    return asset_task_maya_dir(asset_dir, task_name)
...
def _attach_cache_to_node(cache_file_path, obj_node, asset_name=None, task_name="lookdev"):
    ...
    *root-level* node it's being pointed at once more than one is present,
    ...
    """
    _ensure_obj_node_grouped(obj_node, asset_name=asset_name, task_name=task_name)
    resolved = cmds.ls(obj_node, long=True) or []
    if len(resolved) != 1:
        raise RuntimeError(
...
def _shade_asset_namespace_and_label(obj_node):
    ...
    at all, or no longer exists (e.g. the Shade asset it was merged onto
    has since been removed by hand, outside Asset Manager).
    """
...
def clean_rig_asset(*_args):
    ...
    "...") ran into a Maya issue, so this does it as a two-step
    swap instead, per base shape name:
      1. "<name>Shape"          -> "<name>ShapeOrig"   (move the
         original out of the way first, freeing up its name)
      2. "<name>ShapeDeformed"  -> "<name>Shape"        (deformed
         output takes over the now-vacant plain name)
    Only acts on a base name when BOTH a "<name>Shape" and a
    "<name>ShapeDeformed" exist under the selection — a lone "Shape"
    with no "ShapeDeformed" counterpart is left alone (already clean),
    and a lone "ShapeDeformed" with no plain "Shape" counterpart is
    just renamed straight to "<name>Shape" (nothing to move aside).
    """
    ...
    shape_by_base = {}  # base name -> full path of "<base>Shape" node
    deformed_by_base = {}  # base name -> full path of "<base>ShapeDeformed" node
    for shape in shapes:
        short_name = shape.split("|")[-1]
        if short_name.endswith("ShapeDeformed"):
            deformed_by_base[short_name[: -len("ShapeDeformed")]] = shape
        elif short_name.endswith("Shape"):
            shape_by_base[short_name[: -len("Shape")]] = shape

    renamed = 0
    for base, deformed_path in deformed_by_base.items():
        if not cmds.objExists(deformed_path):
            continue

        orig_path = shape_by_base.get(base)
        if orig_path and cmds.objExists(orig_path):
            try:
                cmds.rename(orig_path, f"{base}ShapeOrig")
            except Exception as e:
                cmds.warning(f"Could not rename {base}Shape: {e}")
                continue

        try:
            cmds.rename(deformed_path, f"{base}Shape")
            renamed += 1
        except Exception as e:
            cmds.warning(f"Could not rename {base}ShapeDeformed: {e}")

    if renamed:
        print(f"Clean Rig Asset: cleaned {renamed} shape(s).")
    else:
        cmds.warning('No "...ShapeDeformed" shape nodes were found in the selection.')
    ...
    def on_apply(*_args):
        ...
        for state in row_state.values():
            ...
            if row.get("is_cache_row"):
                ...
                try:
                    _attach_cache_to_node(new_path, cache_node)
                    _tag_cache_attachment(cache_node, row["cache_shot_name"], row["cache_name"], filename=new_filename)
                    applied += 1
                except Exception as e:
        ...
        for item in self.scene_items.values():
            if item["is_cache"]:
                ...
                try:
                    ...
                    cache_dir = get_shot_cache_dir(self.project_path, item["cache_shot_name"])
                    _attach_cache_to_node(os.path.join(cache_dir, new_filename), item["cache_obj_node"])
                    _tag_cache_attachment(
                        item["cache_obj_node"], item["cache_shot_name"], item["cache_name"], filename=new_filename
                    )
                    applied += 1
                except Exception as e:
...
def _task_name_from_scene_path(scene_path):
    ...
    sits at the standard <task>/work/maya/<file> location — the layout
    every asset task (model/rig/lookdev/fx) and every shot task with Maya
    files now uses, see ASSET_TASKS / SHOT_TASK_STRUCTURE — else None.
    Used by Setup Scene and Publish to know which task a scene belongs to
    without asking, just from where it's saved.
    """
    maya_dir = os.path.dirname(scene_path)
    if os.path.basename(maya_dir) != "maya":
        return None
...
def _current_pipeline_package_settings():
    """Current project settings (shot prefix, output size, start frame) — carried along with every export."""
    return {
...
def _apply_pipeline_package_settings(manifest):
    """Restore shot prefix / output size / start frame from a package, and apply to the open scene."""
...
def setup_scene(*_args):
    ...
    nothing for scenes not saved in a Model/Rig/Lookdev task folder (fx
    and texture have no defined group set).
    """
    scene_path = cmds.file(query=True, sceneName=True)
    if not scene_path:
...
def publish_scene(*_args):
    ...
    if not cmds.objExists(export_group):
        cmds.warning(f"Can't publish, {export_group} group not found. Please run Setup Scene first.")
        return

    work_dir = os.path.dirname(scene_path)  # .../<task>/work/maya
    ...
    # .../<task>/work/maya -> .../<task>
    task_dir = os.path.dirname(os.path.dirname(work_dir))
...
def _shot_anim_context_from_scene_path(scene_path):
    ...
    # scene_path: .../shots/<shot>/anim/work/maya/<file>
    maya_dir = os.path.dirname(scene_path)
    work_dir = os.path.dirname(maya_dir)
...
def create_new_asset_model():
    ...
    dest_dir = asset_task_maya_dir(asset_dir, "model")
...
def create_new_asset_task(task_name):
    ...
    dest_dir = asset_task_maya_dir(asset_dir, task_name)
...
# per task — matches the maya subfolder location within SHOT_TASK_STRUCTURE.
...
def show_save_as_window():
    ...
    def compute_shot_preview():
        ...
        if version_choice == "Create New Version":
            ...
            subpath = SHOT_TASK_MAYA_SUBPATH.get(task_name, ["work", "maya"])
...
def _apply_loaded_settings(data):
    """Restore shot prefix / output size / start frame optionVars, and apply them to the open scene."""
