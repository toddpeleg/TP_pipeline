...
def file_increment_and_save():
    """
    Run Maya's built-in Increment and Save (same as File > Increment and
    Save). Always creates a new version, so per the 3.0.0 comment
    feature's rule (only prompt on a versioned save) this always prompts
    for an optional comment first.
    """
    ...
    prompt_scene_comment(do_increment)
